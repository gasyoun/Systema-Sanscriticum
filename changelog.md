...
### Fixed
- **CI на `main` был красным подряд шесть прогонов — `CabinetProbeTest` определял «прод» через `PHP_OS_FAMILY === 'Linux'` (H1931, [#865](https://github.com/gasyoun/Systema-Sanscriticum/issues/865)).** После H1914 `cabinet:probe` включает вердикт `guards:verify`, а `guards:verify` по построению сверяется с живой машиной (`systema-schedule-run.sh`, `cron.service`, `crontab www-data`, `MemoryMax`, `earlyoom`) — но [`config/server_guards.php`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/config/server_guards.php) включал эту проверку по признаку «это Linux», истинному на любом Linux-раннере CI, а не только на проде. Разделение юнит/живой-контур уже существовало (`SystemInspector`, `ServerGuardsAuditor`, `VerifyServerGuardsCommandTest` уже гасит ветку через `config`) — `CabinetProbeTest` просто не унаследовал ту же строку. Исправлено: `'verify_enabled' => env('SERVER_GUARDS_VERIFY', PHP_OS_FAMILY === 'Linux' && env('APP_ENV') !== 'testing')` + `SERVER_GUARDS_VERIFY=false` в `phpunit.xml` как подстраховка (в ряд с `TELEGRAM_SUPPORT_ENABLED`/`PULSE_ENABLED`/`TELESCOPE_ENABLED`). Разрешительный дефолт для прода намеренно НЕ перевёрнут в `false` — пересобранный хост без явной переменной должен проверять предохранители, а не молча переставать. `CabinetProbeTest` зелен (12 passed), Pint чист. Executor: Sonnet 5 (`claude-sonnet-5`).
