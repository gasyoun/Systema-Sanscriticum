...
### Added
- **Дефлекция наконец меряется по обоим каналам, а не по одному Telegram (H1837).** Дневная свёртка `support_daily_rollups` описывала ровно один канал — `telegram_support_chat_id` был NOT NULL, — а поверх неё считают `support:topic-ranking` и `content:detect-gaps`, то есть **рейтинг «что автоматизировать первым» видел только импортированный TG-аккаунт**. Тема, которую спрашивают в основном через виджет на сайте (а также через VK и TG-student-bot — те пишут в те же `chat_messages`, H1200), выглядела в нём маленькой: это смещение измерения, а не пробел в интерфейсе, и оно било ровно в тот отчёт, который существует для выбора приоритета. Строка свёртки получила **`channel`** (`telegram`/`web`, дефолт `telegram` — старые строки считаются ровно как раньше) и nullable `support_conversation_id`; заполнен всегда ровно один из двух FK. Веб-строки считает [`WebChatDailyRollupAggregator`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Services/Support/WebChatDailyRollupAggregator.php) — близнец [`SupportDailyRollupAggregator`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Services/TelegramSupport/SupportDailyRollupAggregator.php), но направление и «человек или ИИ» он **не выводит заново**: свёртка идёт через [`UnifiedMessage::fromChatMessage()`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Support/UnifiedMessage.php), единственный источник правды role-маппинга на веб-стороне — вторая копия дала бы две «правды» о том, что считается ответом куратора. Триггер у каналов разный: Telegram сворачивает сам синк по затронутым датам, у веб-чата синка нет, поэтому ночью в 04:35 идёт `support:rollup-web --days=2` (идемпотентна по паре «тред×день», `--date=`/`--days=` догоняют прошлое). `SupportTopicClassifier` берёт текст дня из того хранилища, которое назвал `channel`, — набор `SupportTopicRule` **общий на оба канала** сознательно: второй, веб-специфичный набор правил дал бы одной теме две категории в зависимости от того, куда написал студент. Потребители разведены явно: кросс-канальные — `support:topic-ranking` (новый `--channel=all|telegram|web` и колонка `web %`, показывающая, куда тема реально приходит) и `content:detect-gaps`; Telegram-скоуп через новый scope `->telegram()` — `SupportDashboardPacketBuilder`, `TelegramSupportAnalytics`, `SupportObservability`, которые рендерят TG-переписку и на веб-строке фаталили бы на `$rollup->chat->telegram_chat_id`. Оба канала видно на **одном экране** без ручной сверки: [`SupportObservability`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Filament/Pages/SupportObservability.php) получил вторую карточку «Веб-чат» с тем же расчётом по каналу `web` — рядом, но **не смешанные** с телеграмными: соседние карточки страницы описывают только userbot, и общая сумма читалась бы как его метрика. Операторская страница разбора переписки [`TelegramSupportAnalytics`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Filament/Pages/TelegramSupportAnalytics.php) остаётся телеграмной по замыслу (она рендерит TG-транскрипты) — закрыто **измерение**, отдельная веб-страница разбора остаётся открытым пунктом [`docs/support-subsystem-map.md`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/docs/support-subsystem-map.md). Единственный НОВЫЙ пишущий путь — ночная команда — за флагом `web_chat_deflection_rollup`, **ВЫКЛ по умолчанию** (`WEB_CHAT_DEFLECTION_ROLLUP`): пока он OFF, не появляется ни одной строки канала `web` и все отчёты считают ровно те же числа, что и до задачи; включение — флаг + разовый догон истории `support:rollup-web --days=N --force`. Читающая сторона флагом не гейтится сознательно — она считает то, что лежит в таблице, и второй рубильник дал бы два рассинхронизированных состояния. Миграция аддитивна и накатывается всегда. Сообщения без `support_conversation_id` (строки до H536) не сворачиваются: ключа группировки у них нет, а домысливать тред по `user_id` задним числом значило бы придумывать данные. Executor: Opus 5 (`claude-opus-5`).
