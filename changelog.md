...
### Added
- **`payments.first_paid_at` + backfill — closes the resurrection-guard audit blind spot (H1645, docs/money-access-core-manual.md §9 C3, [PR #712](https://github.com/gasyoun/Systema-Sanscriticum/pull/712)).** `hasPriorPaidTransition()` used to read only `payment_audits`, which only exists since 08-06-2026 and is never written by the `withoutEvents` create-as-paid paths (silent `PromiseFulfillment::fulfil`, access-only siblings, conditional grants) — so a payment paid-then-reversed before that date, or created paid without an audit snapshot, stayed invisible to the H1359 resurrection guard even with `TOCHKA_WEBHOOK_GUARD` ON. Additive nullable `payments.first_paid_at` (ships in this PR, never run against prod by an agent — programme ruling D16, DEPLOY_QUEUE №60 is Ivan's step) is now stamped once, on the first entry into a paid status: via `Payment::booted()`'s `saving` hook for the normal event-driven path, and directly in the three `withoutEvents` create payloads above (model events are suppressed there, so the hook never fires). `hasPriorPaidTransition()` now checks `first_paid_at` first, falling back to the audit walk — which is itself hardened to inspect **both** old and new status values of each diff (`$status[0]`/`$status[1]`, not just new), so a `paid→failed` row proves prior-paid even with no `created` snapshot. One-shot idempotent artisan `payments:backfill-first-paid-at` (dry-run by default, `--apply` to write, `updateQuietly` — no observer recursion) recovers `first_paid_at` for existing rows from the earliest audit proving prior-paid, falling back to `created_at` for currently-paid rows with no audit trail; a genuinely-traceless pre-08-06-2026 paid-then-reversed payment stays `NULL` and is reported as a residual count, never silently dropped. Tests: `FirstPaidAtTest` (4 — stamp-once, create-as-paid, backfill dry-run vs apply, guard detects a pre-observer paid-then-reversed payment once backfilled) + existing `TochkaWebhookTest` (13) stay green. Full Feature suite green, Pint clean. Executor: Sonnet 5 (`claude-sonnet-5`).
