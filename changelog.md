## [Unreleased]
### Fixed
- **500 при сдаче домашки: страницы ревью-PDF теперь даунскейлятся.** Инцидент 16-08-2026 (16:14–16:21, четыре падения): студент сдал четыре телефонных JPEG по ~5 МБ, а сборка `combined-images.pdf` идёт синхронно в HTTP и вставляла jpeg/png **байт-в-байт** — base64 (+33 %), три живые копии на файл и полноразмерный декод в dompdf съедали 128 МБ FPM. OOM — фатал, мимо `catch (Throwable)` в `rebuildQuietly`; сдача при этом уже сохранена, и студент ретраил, плодя дубли. Лимиты загрузки (10 файлов / 30 МБ / 90 МБ) согласованы с nginx и ни одним числом — с memory_limit. Теперь каждая страница ужимается до `homework.pdf_max_edge_px` (1600) / JPEG q80 (`HOMEWORK_PDF_MAX_EDGE_PX`, `HOMEWORK_PDF_JPEG_QUALITY`): PDF — ревью-копия, оригиналы целы в кабинете. Память постранично; неподъёмная картинка (guard `RasterImageMemory` по габаритам **до** декодирования — извлечён из `CoverToBannerImporter`, копий правды нет) даёт страницу-заглушку, а не 500. EXIF-поворот 3/6/8 включён (dompdf игнорирует Orientation, перекодирование стирает тег). Хвосты: `rebuildQuietly` вынесен за `DB::transaction` в путях удаления файлов, `deleteStudentComment` теперь пересобирает PDF (раньше удалённые картинки жили в PDF вечно), массовый перенос файлов делает 2 пересборки вместо 2N. Побочный плюс: PDF снова влезает в лимит вложения письма (12 МБ). Тесты: `HomeworkImagePdfDownscaleTest` (проверен подстановкой — на старом «как есть» падает), `HomeworkImagesPdfTest`, `HomeworkFlowTest`. Executor: Claude Fable 5 (`claude-fable-5`).
