...
- **H2762: isolated next-step and CTA A/B on Kochergina.** Two default-OFF flags (`CATALOG_NEXT_STEP`, `FLAGSHIP_CTA_AB`) on one named flagship (slug `kocerginoi`). R12: «После этого → Бюлер / тексты / рецитация» on that catalog card only; metric `next_step_click` / `card_impression`. R15: hero label «Смотреть пробный урок» vs «Записаться» (href stays `#sample`); metrics `sample_play` and `begin_checkout`. Guests log to `storefront_events` (not GameEvent, not auth-only `activity_events`). 30-day window; thin n → hide / DEFER, do not ship a winner. Query: `php artisan shop:flagship-experiments --days=7`. Doc: [docs/FLAGSHIP_EXPERIMENTS_H2762.md](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/docs/FLAGSHIP_EXPERIMENTS_H2762.md). Executor: Grok 4.5 (`grok-4.5`).
...
### Fixed
- **Три утверждения модели опровергнуты измерением, а не смягчены.** (1) **P0.3** — SRS не является следом образовательного результата по ступеням: **0 из 17 колод** несут `course_id` или `lesson_id`, «Wave 2» из миграции не выполнена, так что ступень из SRS не выводится; в схеме понижено до следа учебной активности и добавлен измеренный блок `trace_reality`, валидатор которого **требует** от каждого контура назвать, на что тот ответить не может ([PR #1614](https://github.com/gasyoun/Systema-Sanscriticum/pull/1614)). (2) **P0.1 + Q1** — F1 и F4 не разделены и на уровне записи: все **36 из 36** реплик, несущих F4, несут и `new_status`, то есть развивающей обратной связи без приложенного акта оценивания в корпусе нет; но **19 реплик несут статус без вердикта в тексте**, значит слияние живёт в акте авторства, а не в схеме, и вопрос «сохранится ли F4, если статус перестанет ставиться тем же действием» — про интерфейс проверки ([PR #1615](https://github.com/gasyoun/Systema-Sanscriticum/pull/1615)). (3) **Q1b** — заявление Q1 «корпус хинди, а не санскрит» было слишком сильным: фактически **82,7 % хинди и 17,3 % санскрит** (Кочергина, гр. 60/61) ([PR #1617](https://github.com/gasyoun/Systema-Sanscriticum/pull/1617)).
...
### Changed
- **`game_events` получила nullable `user_id` — прямая отмена контракта H1360/H1678 «никакого идентификатора студента», авторизованная §1 плана (R4-2, MG 10-08-2026).** Причина: зафиксированный KPI D6 мерил вход в воронку, а вопрос «играл → сдал ДЗ» на авторизованных студентах связать было нечем. **Столбец пишется только на сервере из веб-сессии (`'user_id' => $user?->id`), анонимный хит остаётся `NULL`, клиент влиять на значение не может** — и это не заявление, а два теста: `anonymous_hit_leaves_user_id_null` и `user_id_is_taken_from_the_session_and_never_from_the_client`, где клиент присылает `user_id: 999999`, а сервер его игнорирует. Прежнее blanket-утверждение «столбца user_id нет» в [GamesFunnelTelemetryTest](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/tests/Feature/GamesFunnelTelemetryTest.php) заменено этими двумя адресными проверками, а не просто удалено. IP и user-agent по-прежнему отсутствуют **намеренно** (R20 / 152-ФЗ) — расширение области на них не производилось. В докблоке [GameEvent](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Models/GameEvent.php) записано и то, чего делать нельзя: `ctaRegistrationRate()` обязана и дальше считать по флагу `authenticated` и **не** джойниться по `user_id`, иначе анонимные клики по CTA (у которых там `NULL`) выпадут из знаменателя и метрика тихо завысится. Executor: Opus 5 (`claude-opus-5`).
