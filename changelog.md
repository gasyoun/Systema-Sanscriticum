...
### Fixed
- **`no-manifest` из H2618 — НЕ продуктовый дефект: значок установки PWA появляется у живого пользователя.** [H2618](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2618-Sonnet_Systema-Sanscriticum_offline-cabinet-w0-windows-edge-pwa-verification_12.08.26.md) получил `Page.getInstallabilityErrors → {"errorId":"no-manifest"}` при спец-валидном манифесте и пятью попытками починки причину не нашёл, оставив человеку вопрос: реальный дефект или артефакт CDP-навигации. Проверено 13-08-2026 вручную — [https://samskrte.ru/dvaram](https://samskrte.ru/dvaram) под логином в **Chrome на Windows 10**: **значок установки в омнибоксе появляется.** Дорогая ветка закрыта: устанавливаемость PWA у всех пользователей во всех Chromium-браузерах НЕ сломана, и правки продукта эта строка Wave 0 не требует. **Чего проверка не доказывает — сказано прямо:** клик был Chrome/Windows 10, а строка H2618 — Edge/Windows 11, поэтому две более узкие гипотезы (артефакт CDP-сессии либо что-то специфичное для Edge/локального dev-сервера H2618) этим тестом не различаются; обе — проблема ИЗМЕРЕНИЯ, а не продукта, поэтому ничего не блокируют. Один конфаундер по дороге устранён: до 14:55:39Z 13-08-2026 прод отдавал манифест как `application/octet-stream`, после прогона [scripts/nginx_webmanifest_mime.sh](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/scripts/nginx_webmanifest_mime.sh) — как `application/manifest+json`; объяснением расхождения это не является (H2618 получил `no-manifest` и при правильном типе), но клик шёл уже по исправленному заголовку. **Следствие для незакрытых строк:** манифест, сервис-воркер и устанавливаемость подтверждены на живом Chromium, поэтому на Android-устройстве человеческий шаг установки должен просто сработать, и риск строки Android Chrome PWA теперь целиком в пяти V0-проверках, а не в установке приложения. Executor: Opus 5 (`claude-opus-5`).
- **H2634 B1 — сервис-воркер кабинета удалял ЧУЖИЕ кэши при каждой активации, включая зашифрованное офлайн-содержимое ([#1630](https://github.com/gasyoun/Systema-Sanscriticum/issues/1630)).** [public/sw.js](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/public/sw.js) в `activate` сносил всё, кроме `ors-cabinet-shell-v1`, — а офлайн-кабинет пишет шифротекст в соседний кэш `systema-offline-spike-encrypted-v1` ([resources/js/offline/spike.ts](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/resources/js/offline/spike.ts)). `install` зовёт `skipWaiting()`, `activate` — `clients.claim()`, поэтому активация происходит **немедленно при любой правке байта** воркера: обычный деплой молча уничтожал скачанные уроки, тогда как IndexedDB продолжала считать их скачанными. Deny-list заменён **версионированным allow-list по своему пространству имён** (`ors-cabinet-shell-`): устаревшие СВОИ версии по-прежнему подчищаются, всё за пределами префикса не трогается. Содержимое сознательно **не** переехало в кэш оболочки — это воспроизвело бы ту же путаницу владения через один релиз. Пин — [resources/js/offline/sw-cache-migration.test.ts](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/resources/js/offline/sw-cache-migration.test.ts) (`npm run test:sw-cache-migration`): тест исполняет **сам закоммиченный `public/sw.js`**, а не копию его логики, засевает соседний кэш содержимого, активирует поверх него воркер НОВОЙ версии и проверяет, что шифротекст жив. **5 зелёных на исправленном воркере; 4 из 5 падают на доисправленном** — то есть это регрессионный гейт, а не описание уже верного поведения. Executor: Opus 5 (`claude-opus-5`).

### Added
- **H2619 (Opus 5, `claude-opus-5`) — Offline cabinet Wave 0: iPhone-ряд закрыт как BLOCKED, и найдены три дефекта в трекаемой конфигурации, которые устройство бы не спасло.** Обе полосы — installed Safari PWA и Capacitor remote-origin WKWebView — получили раздельный вердикт **BLOCKED — host missing** с чеками команд (исполнитель на Windows: нет macOS/Xcode/CocoaPods/iPhone), поэтому версии устройства/iOS/Safari/WKWebView не зафиксированы и ничем не заменены. Существенное здесь не чек хоста, а статический аудит: **B1** — `activate` в `public/sw.js` удаляет все кэши, кроме `ors-cabinet-shell-v1`, включая `systema-offline-spike-encrypted-v1`, то есть любое обновление service worker молча уничтожает скачанный зашифрованный контент; **B2** — `server.url` (удалённый origin) против `errorPath: 'error.html'` из локального бандла: офлайн-фолбэк живёт на другом origin и по same-origin policy не видит ни ключ в IndexedDB, ни шифротекст в Cache Storage, так что холодный офлайн-старт упирается в экран «Повторить»; **B3** — трекаемой реализации `OfflineCrypto` не существует нигде, кроме типа и fail-closed пробы, поэтому Keychain-гейт на устройстве недостижим. Каждый из трёх сам по себе даёт STOP на полностью укомплектованном iPhone — чинить их дешевле сейчас, чем на арендованном macOS. Wave 1 не разблокирована, шифрование не ослаблено, plaintext fallback не добавлен, native-проекты не тронуты. Evidence: `docs/OFFLINE_CABINET_WAVE0_IPHONE_EVIDENCE_2026-08-12.md`.
- **H2634 B2/B3 — Capacitor-обёртка ОСТАНОВЛЕНА для офлайн-содержимого; `OfflineCrypto` остановлен вместе с ней.** `server.url` держит WebView на удалённом origin кабинета — там и создаются device key и зашифрованные чанки; `server.errorPath` отдаётся из локальной сборки (`capacitor://localhost` на iOS), и same-origin policy закрывает офлайн-странице доступ и к ключу, и к чанкам. Capacitor держит **один** origin на WebView, поэтому настройками это не примиряется, а единственный выход — выбросить `server.url` ради локального бандла с кросс-origin загрузкой — и есть тот нативный переписыватель, который запрещают platform gate архитектуры и reuse-решение H824. Решение записано в [docs/ARCHITECTURE_SYSTEMA_OFFLINE_CABINET.md](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/docs/ARCHITECTURE_SYSTEMA_OFFLINE_CABINET.md) § Load-bearing platform gate вместе с отвергнутыми выходами и условием пересмотра. **Это остановка средства доставки, а не способности:** установленные PWA (Android Chrome, iPhone Safari, Windows Edge) same-origin от начала до конца и остаются в области V0, поэтому последствий для сессии и авторизации **нет** — офлайн-читалка продолжает жить на переиспользуемой веб-сессии, второго контура аутентификации не заводится. Запрет механический, а не декларативный: `assertOfflineStorageAllowed()` в [spike.ts](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/resources/js/offline/spike.ts) отказывается создавать `CacheEncryptedChunkStore` и возобновлять загрузку при `Capacitor.isNativePlatform()`, так что «второе хранилище исподволь», запрещённое архитектурой, обычным путём кода недостижимо. B3 закрыт как **не подлежащий реализации**: мост существует ради ключа внутри обёртки, у остановленной обёртки потребителя нет, и `probeNativeCrypto()`, возвращающий all-false на нативной платформе, — теперь правильное установившееся состояние, а не пробел; писать некомпилируемую и непроверяемую нативную криптографию на Windows-хосте без устройства значило бы превратить честный стоп в непроверенное заявление о реализации. `npm run test:offline-spike` — **8 зелёных** (было 6). Из-за этого решения строка WKWebView в V0 переведена из «ожидает устройства» в «остановлена»: macOS/Xcode для Wave 0 больше не нужны. Executor: Opus 5 (`claude-opus-5`).
...
- **H2617 (Opus 5, `claude-opus-5`) — Android/WebView V0-строка Wave 0 закрыта вердиктом `BLOCKED — host missing`, а не PASS.** На исполняющем хосте нет ни одной части требуемого тулчейна: `java` и `adb` не найдены ни в PATH, ни рекурсивным поиском `java.exe`/`adb.exe` по четырём корням (0 попаданий), `ANDROID_HOME`/`ANDROID_SDK_ROOT`/`JAVA_HOME` пусты, Android Studio отсутствует, среди USB-устройств нет ни одного телефона. Эмуляторный запасной путь закрыт **независимо**: `VirtualizationFirmwareEnabled: False`, нет SLAT, 11.7 ГБ свободно — то есть аппаратное ускорение выключено на уровне BIOS/UEFI, а на неускоренном образе гейты «≤2 с холодного открытия» и «≤20 % накладных» дали бы выдуманные числа, а не измерение. Все пять V0-проверок записаны как **UNATTEMPTED**; `npm run test:offline-spike` сознательно **не** перезапускался и не выдавался за Android-доказательство. **H2617 гонялся ДО того, как H2634 попал в ветку** (смёржен 13-08-2026 10:43Z, [PR #1636](https://github.com/gasyoun/Systema-Sanscriticum/pull/1636), уже после того, как эта ветка отошла от `0979f025`), поэтому измерения верны, а их **область** — нет, и запись поправлена по факту рулинга: **Android-цель распадается на две дорожки**. Дорожка B (Capacitor Android WebView) **ОСТАНОВЛЕНА** рулингом B2 — Capacitor держит один origin на WebView, и WebView-дорожка Android остановлена ровно по той же причине, что и WKWebView на iPhone; сессия с устройством за неё не должна. Дорожка A (installed Chrome PWA) остаётся `BLOCKED — host missing`, но **непреграждённой**: B1 починен, B2/B3 к same-origin PWA не применяются. Практическое следствие — **весь инвентарь тулчейна выше относится к остановленной дорожке**: для installed Chrome PWA не нужны ни JDK, ни SDK/API 36, ни ADB, ни виртуализация, ни ~20 ГБ диска — нужен **телефон с Chrome, и больше ничего**. Побочная находка «**`OfflineCrypto` bridge не существует как код**» (`git grep` на ветке даёт только прозу) как измерение верна, но как пробел **отменена**: B3 постановил, что мост **не нужен** — он защищал ключ внутри Capacitor-обёртки, а B2 эту обёртку для офлайн-контента остановил, потребителя нет, и `probeNativeCrypto()`, возвращающий all-false на нативной платформе, теперь **правильное установившееся состояние**, а не дыра. Wave 1 не разблокирована. Evidence: `docs/OFFLINE_CABINET_WAVE0_ANDROID_EVIDENCE_2026-08-12.md`.
