...
"@
    ...
    if (Test-Path $here) {
        ...
        if ($left.Count -eq 0) {
            # Без обратных кавычек: в двойных кавычках PowerShell трактует ` как
            # escape, и завершающая ` съедала закрывающую кавычку — файл переставал
            # парситься целиком (поймано на живом прогоне 30-07-2026).
            Warn "каталог опустел, но Windows ещё держит саму папку — работы в ней нет, git о ней не знает; исчезнет сама, либо снесите её: Remove-Item -Recurse -Force $here"
