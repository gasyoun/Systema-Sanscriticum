...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Ревью-PDF из картинок (combined-images.pdf)
    |--------------------------------------------------------------------------
    |
    | PDF собирается СИНХРОННО в HTTP-запросе сдачи, поэтому его аппетит обязан
    | помещаться в memory_limit php-fpm (на проде 128M — сверяйте при правке!).
    | Лимиты загрузки выше согласованы с nginx/post_max_size, но НЕ с памятью:
    | 16-08-2026 четыре телефонных фото по ~5 МБ, вставленные в PDF в исходном
    | разрешении, уронили сборку по OOM, и студент увидел 500 при уже
    | сохранённой сдаче. Теперь каждая страница даунскейлится до
    | pdf_max_edge_px по длинной стороне и перекодируется в JPEG: это
    | ревью-копия, оригиналы не трогаются и доступны в кабинете. Неподъёмная
    | картинка не роняет сборку — вместо неё страница-заглушка
    | (см. RasterImageMemory).
    |
    */

    // Длинная сторона страницы PDF, px. 1600 ≈ читаемый рукописный текст на A4.
    'pdf_max_edge_px' => (int) env('HOMEWORK_PDF_MAX_EDGE_PX', 1600),

    // JPEG-качество страниц PDF. 80 — баланс «читаемо / влезает в письмо
    // (MAIL_ATTACH_MAX_BYTES = 12 МБ)»; ниже начинают плыть тонкие штрихи.
    'pdf_jpeg_quality' => (int) env('HOMEWORK_PDF_JPEG_QUALITY', 80),
