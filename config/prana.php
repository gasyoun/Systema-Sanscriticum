...
return [
    ...
    // srs_review на пользователя — фарм батчами/реплеями ограничен потолком,
    // легитимная нагрузка (20 новых карточек × несколько колод + просроченные)
    // с запасом покрыта. Состояние повторений и журнал пишутся без потолка;
    // 0 или меньше = потолок снят.
    'srs_review_daily_cap' => (int) env('PRANA_SRS_REVIEW_DAILY_CAP', 120),
