...
return [
    ...
    'faq_rag' => [
        ...
        /*
         | H5065: сколько ПОЛНЫХ разделов уходит отвечающей стороне (личка
         | саппорта, полоса Telegram Business). Отдельный ключ от bot_top_k,
         | потому что бюджеты разные: кабинетный бот кладёт контекст в
         | системный промпт и уже платит за персону с каталогом, а личка
         | отвечает цитатой раздела и держит контекст компактнее.
         |
         | Берём 6: до H5065 отвечающая полоса видела 3 СНИППЕТА по 280
         | символов — то есть меньше одного раздела целиком.
         */
        'answer_top_k' => (int) env('SUPPORT_FAQ_RAG_ANSWER_TOP_K', 6),
        /*
         | Потолок символов контекста в промпте отвечающей стороны. Обрезка
         | идёт по границе раздела (KnowledgeContext::promptBlockCapped).
         | 0 = без потолка. 12000 символов ≈ 4 тыс. токенов: при шести
         | разделах faq.md потолок не достигается, он сторожит распухший
         | корпус и растущий faq_from_lectures.md.
         */
        'answer_max_chars' => (int) env('SUPPORT_FAQ_RAG_ANSWER_MAX_CHARS', 12000),
     ...
     */
    'llm_replies' => [
        // Порог BM25-скора лучшего попадания, ниже которого LLM-ветка
        // отказывается формулировать: формулировать не из чего, а отвечать
        // «по памяти» модели рулинг R3 запрещает. Отдельный ключ, а не
        // shadow_min_score: теневые пороги B5 калибровались ПОКАТЕГОРИЙНО,
