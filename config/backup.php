...
return [
    ...
    'backup' => [
        ...
        // >1 ГБ, а сотни МБ иногда «сохраняются» 2xx без записи (чёрная дыра,
        // прод 22–23-08-2026: три «успеха» на ~2 ГБ, части 404; класс ~50 МиБ
        // при этом доезжает честно). Восстановление: скачать все части одной
        // группы, склеить `cat *.part-*-of-*.zip > backup.zip`, распаковать.
        'split_upload' => [
            // Куда льём части (должен совпадать с диском из destination.disks,
            // который НЕ local).
            'disk' => 'yandex_disk',
            // Размер части. 50 МиБ = эмпирически проходящий класс (проба
            // probe-50mb.bin легла целиком); порог guards BACKUP_MIN_ARCHIVE_MB
            // в scripts/server_guards.conf стоит ниже этой величины.
            'max_part_mb' => (int) env('BACKUP_SPLIT_PART_MB', 50),
