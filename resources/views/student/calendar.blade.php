...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-nunito">
    ...
    <div class="mb-8 md:mb-10">
        <h1 class="text-3xl md:text-4xl font-extrabold text-[#101010] tracking-tight mb-2">
            Расписание занятий
        </h1>
                    ...
                    @foreach($events as $event)
                        ...
                        <div class="bg-white rounded-2xl border {{ $isLive ? 'border-[#E85C24]/60 shadow-[0_10px_30px_rgba(232,92,36,0.15)]' : 'border-gray-100' }} p-4 md:p-5 hover:shadow-[0_10px_25px_rgba(0,0,0,0.06)] hover:border-[#E85C24]/30 transition-all duration-300 group flex flex-col h-full relative overflow-hidden">
                            ...
                            <div class="flex justify-between items-start gap-2 mb-3 {{ $isLive ? 'pr-24' : '' }}">
                                ...
                                @if($event->course)
                                    <div class="text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-gray-400 border border-gray-100 px-2 py-1 rounded bg-gray-50 truncate max-w-[50%]"
                            ...
                            @if($event->description)
                                <p class="text-gray-500 text-xs md:text-sm line-clamp-2 mb-3">
                                    {{-- Прячем «голую» ссылку, если она дублирует поле link --}}
                                    {{ \Illuminate\Support\Str::of($event->description)->replaceMatches('~https?://\S+~i', '')->trim() }}
                                </p>
                            @endif
                            ...
                            <div class="mt-auto pt-4 border-t border-gray-50 space-y-2">
                                ...
                                {{-- Если ничего нет — мягкий placeholder --}}
                                @if(!$event->link && !$event->course)
                                    <div class="text-center text-xs text-gray-400 italic py-2">
                                        Подробности уточняйте у куратора
                                    </div>
                                @endif
                            </div>
                        </div>
        ...
        @empty
            ...
            <div class="text-center py-16 bg-white rounded-[2rem] border border-dashed border-gray-200 shadow-sm mt-8 max-w-2xl mx-auto">
                ...
                <p class="text-gray-500 text-base max-w-sm mx-auto">
                    В ближайшее время в вашем расписании нет запланированных вебинаров или встреч.
                </p>
            </div>
        @endforelse
    </div>
