...
@php
    // Состав слайдов зависит от включенных флагов: перечисляем только то,
    ...
    $tourSteps = [
        ...
        [
            ...
            'text' => 'Кнопка «Продолжить» ведет прямо в следующий урок, а прогресс-бар на карточке курса показывает, сколько уже пройдено.',
        ],
        [
    ...
    if ($tourSrsOn) {
        $tourSteps[] = [
            ...
            'text' => 'Слова закрепляются интервальными повторениями — разделы «Карточки», «Мои колоды» и «Статистика карточек».'
                . ($tourDrillsOn ? ' Короткие разминки — в «Тренажерах».' : ''),
        ];
    }
...
<div x-data="{
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85dvh] overflow-y-auto custom-scrollbar relative"
        ...
        {{-- Ноги: счетчик слайдов + навигация. Телеметрия только кликами (impression
