@php
    ...
    // Курс формата «прием есть, проверки нет» (H3081): обещать «На проверке»
    ...
    // ДЗ «включено», но преподаватель еще не сформулировал условие и студент
            ...
            <form action="{{ route('student.homework.store', [$course->slug, $lesson->id]) }}" method="POST" enctype="multipart/form-data"
                x-data="{
                    ...
                    add(input) {
                        ...
                        // H4294: тяжелый файл (длинное видео с телефона) не должен
                        // стартовать в загрузку — nginx убьет тело голой 413, телефон
                        // сольет трафик впустую, а ни серверная валидация, ни
                    ...
                    oversizeHint() {
                        ...
                        return `Не добавлены: ${names}. На один файл — до ${this.maxFileMb} МБ, на всю отправку — до ${this.totalMaxMb} МБ. Тяжелое видео обрежьте до 2–3 минут или сожмите, рукописную работу можно прислать фото.`;
                    },
                }">
                @csrf
                @if($hwIsRework)
