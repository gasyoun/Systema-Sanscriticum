    ...
    <div class="mb-6 rounded-2xl border border-violet-100 bg-white p-5 md:p-6 shadow-sm">
        <div class="flex items-center gap-3 mb-4">
            ...
            <div>
                ...
                <p class="text-gray-500 text-sm">Потратьте прану на перки. Ваш баланс: <span class="font-bold text-brand"><x-prana-lotus /> {{ number_format($balance, 0, '.', ' ') }}</span></p>
            </div>
        </div>
            ...
            @foreach($perks as $perk)
                ...
                <div class="rounded-xl border border-gray-100 p-4 flex flex-col {{ $available ? '' : 'opacity-60' }}">
                    ...
                    <div class="mt-3 flex items-center justify-between gap-2">
                        <span class="font-extrabold text-brand tabular-nums"><x-prana-lotus /> {{ number_format($perk->cost, 0, '.', ' ') }}</span>
                        @if(! $available)
