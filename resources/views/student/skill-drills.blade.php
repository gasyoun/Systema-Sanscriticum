...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 font-nunito">
    ...
    <div class="mb-8 mt-6">
        ...
        <p class="text-gray-500 text-lg max-w-3xl">
            ...
            (для этого есть <a href="{{ config('srs.enabled') ? route('student.srs') : '#' }}" class="text-[#E85C24] font-bold hover:underline">Карточки</a>),
        ...
        @foreach($drills as $drill)
            <a href="{{ $drill['url'] }}" target="_blank" rel="noopener"
               class="group bg-white rounded-2xl border border-gray-200 p-5 hover:border-[#E85C24]/40 hover:shadow-[0_8px_30px_rgba(232,92,36,0.08)] hover:-translate-y-0.5 transition-all duration-200 flex flex-col">
                ...
                <span class="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md bg-orange-50 text-[#E85C24] border border-orange-200 mb-3 self-start">
                    {{ $drill['family'] }}
                </span>
