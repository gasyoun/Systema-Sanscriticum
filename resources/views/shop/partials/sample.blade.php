...
<section id="sample" class="mb-16 lg:mb-20" data-analytics="sample-lesson">
    ...
    <a href="{{ route('shop.course.preview', $course->slug) }}"
       class="group block rounded-3xl overflow-hidden bg-[#111622] border border-[#1F2636] hover:border-brand/50 transition-all duration-300">
        <div class="relative aspect-video w-full bg-gradient-to-br from-[#1A2235] to-[#0A0D14] flex items-center justify-center overflow-hidden">
            ...
            @endif
            <span class="relative flex items-center justify-center w-20 h-20 rounded-full bg-brand text-white shadow-[0_0_40px_rgba(232,92,36,0.5)] group-hover:scale-110 transition-transform">
                <i class="fas fa-play text-2xl ml-1"></i>
            </span>
        </div>
        <div class="flex items-center justify-between gap-4 p-5">
            <div class="min-w-0">
                <div class="text-[11px] font-black uppercase tracking-widest text-brand mb-1">Смотреть пробный урок</div>
                <div class="text-lg font-bold text-white truncate">{{ $course->previewLesson->title }}</div>
            </div>
            <i class="fas fa-arrow-right text-slate-500 group-hover:text-brand group-hover:translate-x-1 transition-all shrink-0"></i>
        </div>
    </a>
</section>
@endif
