{{--
    Переопределение filament-actions::group.
    Единственное отличие от вендорного шаблона — у выпадающего меню действий
    УБРАН `teleport`. Под кастомным «Масштабом таблиц» (CSS zoom на .fi-ta-content)
    телепортируемая в <body> плашка считала координаты по zoom-кнопке, но сама
    жила в немасштабированном body и «съезжала». Без teleport меню рендерится
    внутри той же zoom-таблицы и позиционируется корректно.
    При обновлении Filament сверять с vendor/filament/actions/.../components/group.blade.php.
--}}
@props([
    'actions' => [],
    'badge' => null,
