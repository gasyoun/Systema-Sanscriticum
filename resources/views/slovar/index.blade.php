...
@section('content')
    ...
    <div class="flex flex-wrap justify-center gap-2 mb-10">
        <a href="{{ route('slovar.index') }}"
           class="px-4 py-2 rounded-xl text-sm font-bold transition-colors {{ ! $activeDictionary ? 'bg-brand text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700' }}">
            Все словари
        </a>
        @foreach($dictionaries as $dictionary)
            <a href="{{ route('slovar.index', ['dict' => $dictionary->id]) }}"
               class="px-4 py-2 rounded-xl text-sm font-bold transition-colors {{ $activeDictionary?->id === $dictionary->id ? 'bg-brand text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700' }}">
                {{ $dictionary->name }}
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            @foreach($words as $word)
                <a href="{{ route('slovar.show', $word->slug) }}"
                   class="block bg-[#161b28] border border-gray-700/60 rounded-xl px-4 py-3 hover:border-brand/60 hover:-translate-y-0.5 transition-all">
                    @if($word->devanagari)
                        <div class="deva text-lg text-white leading-tight">{{ $word->devanagari }}</div>
                    @endif
                    <div class="text-sm text-brand font-semibold truncate">{{ $word->iast ?: $word->cyrillic }}</div>
                </a>
            @endforeach
        </div>
