{{--
    ...
    остатка»: решение человека не должно раствориться в расчете (§4 PLAN).
...
<table>
    ...
    @empty
        <tr><td colspan="4">Подтвержденных выплат по этой семье потоков нет.</td></tr>
    @endforelse
    <tr class="total">
...
@unless ($attributionConfirmed)
    <div class="note">
        ...
        {{ \App\Support\Plural::ru(count($pending), 'платеж', 'платежа', 'платежей') }}
        на {{ $money($pendingTotal) }} ₽ проведены по этим курсам как «Расход» и еще не размечены.
