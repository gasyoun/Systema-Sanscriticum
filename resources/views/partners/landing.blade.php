    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 py-10 md:py-16">
        ...
        <section class="text-center mb-12">
            ...
            <p class="text-gray-600 text-lg max-w-2xl mx-auto">
                ...
                @if($rewardPercent > 0)
                    <span class="font-bold text-[#101010]">{{ str_replace('.', ',', rtrim(rtrim(number_format($rewardPercent, 1, '.', ''), '0'), '.')) }} %</span>
                    от первого платежа приведенного клиента.
                @else
                    <span class="font-bold text-[#101010]">{{ number_format($reward, 0, '.', ' ') }} ₽</span>
                    за каждого приведенного клиента.
                @endif
                Всю остальную работу берем на себя.
            </p>
        </section>
        ...
        <section class="grid md:grid-cols-3 gap-4 mb-12">
            @foreach([
                ...
                ['fa-ruble-sign', 'Вы получаете выплату', 'Когда приведенный клиент впервые оплачивает курс, вам начисляется вознаграждение к выплате — по условиям ниже.'],
            ] as [$icon, $title, $text])
        ...
        <section class="mb-12 rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <dl class="divide-y divide-gray-100">
                @foreach([
                    ['Вознаграждение', $rewardPercent > 0
                        ? str_replace('.', ',', rtrim(rtrim(number_format($rewardPercent, 1, '.', ''), '0'), '.')).' % от первого платежа приведенного клиента'
                        : number_format($reward, 0, '.', ' ').' ₽ за каждого приведенного клиента'],
