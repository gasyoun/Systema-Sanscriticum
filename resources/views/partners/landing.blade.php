...
<head>
    ...
    @php
        $pageTitle = 'Партнёрская программа — Общество ревнителей санскрита';
        $description = 'Рекомендуйте наши курсы и получайте вознаграждение за каждого приведённого клиента. Все условия и цифры — сразу при регистрации.';
    @endphp
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 py-10 md:py-16">
        ...
        <section class="text-center mb-12">
            <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#E85C24]/10 text-[#E85C24] text-sm font-bold mb-4">
                <i class="fas fa-handshake"></i> Партнёрская программа
            </span>
            ...
            <p class="text-gray-600 text-lg max-w-2xl mx-auto">
                Мы ценим каждую связь, которая сложилась за время работы, — и предлагаем вывести её
                ...
                за каждого приведённого клиента. Всю остальную работу берём на себя.
            </p>
        </section>
        ...
        <section class="grid md:grid-cols-3 gap-4 mb-12">
            @foreach([
                ...
                ['fa-user-graduate', 'Клиент учится', 'Мы принимаем клиента, консультируем и ведём его сами — от вас нужна только рекомендация.'],
                ['fa-ruble-sign', 'Вы получаете выплату', 'Когда приведённый клиент впервые оплачивает курс, вам начисляется фиксированное вознаграждение к выплате.'],
            ] as [$icon, $title, $text])
        ...
        <section class="mb-12 rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <dl class="divide-y divide-gray-100">
                @foreach([
                    ['Вознаграждение', number_format($reward, 0, '.', ' ').' ₽ за каждого приведённого клиента'],
                    ['Когда начисляется', 'При первой реальной оплате курса приведённым клиентом'],
                    ['Учёт', 'Прозрачно: в личном кабинете партнёра и в Telegram-боте видно каждого клиента и статус выплаты'],
                    ['Как платим', 'Реальная выплата на согласованные реквизиты (карта / СБП). Не кредит на покупку.'],
                    ['Что от вас нужно', 'Только рекомендация. Продажу, поддержку и обучение ведём мы.'],
                ] as [$term, $val])
        ...
        <section id="join" class="rounded-2xl border border-orange-100 bg-gradient-to-br from-orange-50 to-white p-6 md:p-8">
            ...
            <p class="text-gray-600 mb-6">Заполните заявку — мы активируем ваш партнёрский аккаунт и вышлем ссылку для рекомендаций.</p>
