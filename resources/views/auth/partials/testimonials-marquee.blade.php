...
<script>
    ...
    // Случайная стартовая точка колонок: сервер ставит фиксированные задержки, а тут
    // каждая колонка «уже едет» случайное время — при каждом заходе видны другие отзывы.
    // Длительность берём из --lt-dur той же колонки; без JS остаются серверные задержки.
    document.querySelectorAll('.lt-col').forEach(function (col) {
        const dur = parseFloat(col.style.getPropertyValue('--lt-dur')) || 0;
        if (dur > 0) col.style.setProperty('--lt-delay', -(Math.random() * dur).toFixed(1) + 's');
    });
