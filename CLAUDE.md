...
| Витрина курса (`CourseCadence`) | Курс может идти **несколькими потоками** одной программы — [`CourseCadence`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Support/CourseCadence.php) группирует расписание по `schedules.group_id`. **Никогда не суммируй потоки:** `total()`/`hours()` берут длиннейший поток, `progressLabel()` при нескольких потоках возвращает `null` (общий остаток не описывает ни одного студента — каждый ходит ровно в один поток), потоки называются построчно через `streamLines()`. Дефект спит, пока календарь есть только у одного потока. Закреплено [`CourseCadenceMultiStreamTest`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/tests/Feature/Shop/CourseCadenceMultiStreamTest.php). | — |
