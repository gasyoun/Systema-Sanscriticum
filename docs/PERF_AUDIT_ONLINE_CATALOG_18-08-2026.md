...
|---|---|---|---|
| 1 | ~~Конвертация обложек в WebP + миграция 89 существующих~~ — **сделано 18-08-2026**, см. ниже | 27,6 МБ → ~2,5 МБ на полной прокрутке | — |
...
### Пункт 1 закрыт 18-08-2026

Обложки переводятся в WebP **сами** — и уже залитые, и все будущие. Разовой
конвертации было бы мало: следующая обложка из админки вернула бы PNG. Рубежа
два и ни один не требует человека — наблюдатель на сохранении курса и ежедневная
`media:covers-to-webp` для всего, что прошло мимо Eloquent. Кодек — GD
`imagewebp`, новой зависимости не понадобилось (Intervention/Image из исходного
плана не нужен: WebP на проде уже есть).

Устройство, список сознательно НЕ конвертируемых картинок (Telegram принимает
`.webp` как стикер!) и грабли — в
[docs/MEDIA_WEBP_AUTOCONVERSION_2026.md](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/docs/MEDIA_WEBP_AUTOCONVERSION_2026.md).

Оставшиеся пункты 2–4 не конфликтуют с H2826 (пагинация каталога).
