...
требующие 8.2/8.3, и они упадут на 8.1 (платформенный чек сейчас отключён и не защитит).

---

## 0. Предполётная фиксация (записать ДО изменений — пригодится для отката)
...
```bash
composer update --no-dev -o      # пересоберёт lock честно под 8.3
```
...
- Платёжный вебхук Точки и Telegram-вебхук кабинетного бота принимают POST.
- Redis-кэш: проверить раздел с числовыми кэшами (известный баг — RedisStore отдаёт число
...
Старый PHP 8.1 остаётся установленным, поэтому откат — это вернуть 3 указателя:
```bash
