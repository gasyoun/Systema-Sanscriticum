...
недонабрана». Сейчас это чисто ручной процесс — нет системного объекта «идёт набор» и
...
| Дата, от которой считать «за N дней» | **Обе** — плановая дата по умолчанию (`planned_start_date`), ручной override при переносе (`start_date_override`) перезапускает отсчёт |
...
| Точечная рассылка студентам в мессенджер (не кураторам) | ✅ `SendMessengerAlerts` job — шлёт по `telegram_id`/`vk_id`, пропускает без мессенджера | [tests/Feature/ClassReminderTest.php](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/tests/Feature/ClassReminderTest.php) |
...
настроек) уже есть. Не хватает четырёх вещей:
...
3. Таблица предпочтений по времени (если её правда нет — см. открытый вопрос).
4. Scheduled-команда + шаблон уведомления «за N дней до старта, группа ещё недобрана».
...
planned_start_date  date    nullable             // ожидаемая дата — дефолт для отсчёта «за N дней»
...
механизм найдётся, эта таблица не нужна — годится импорт/интеграция вместо новой сущности.
...
(включая уже оплативших через `Payment.course_id`, даже если формально ещё не в `group_user` —
...
вопросом — как только наберём минимум, сообщим точную дату» — **не молчание**, а честный
...
- Симметричное дополнение `CuratorNotifier`: `groupUnderEnrolled(Group $group)` — сообщение
  ...
  шлёт студентам) — куратор видит проблему одновременно со студентами, а не узнаёт из
  входящего вопроса.

## Вне объёма этой спеки (следующие развилки)

- Кто именно получатель уведомления — участники ещё-не-существующей группы или все
