...
| **GC-C2** manager sales attribution | C | **SHIPPED** (H2058 residual, 03-08-2026) | 2 (§4) — shipped | F5 RULED 02-08-2026 implemented as specified: new [`ManagerSalesReport`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Filament/Pages/ManagerSalesReport.php) page (separate from `OrderPaymentConversion`, §4.3), `OrderPaymentConversionService::managerScoreboard()` groups by `payments.created_by_user_id` (not `leads.assigned_to`), `RoleGate::managerSalesReport()` for F5(b) visibility. Flag `manager_sales_report` default `false`. Tests: `ManagerSalesReportTest` (6). |
