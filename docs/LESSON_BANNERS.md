...
_Created: 22-09-2026 · Last updated: 22-09-2026 (номер в кружке, водяной номер под верхним слоем)_
...
Скрипт сам находит шрифты PSD — в системных шрифтах Windows и в установленных «только для меня»; другие папки добавляются `--fonts-dir` (можно несколько раз). Найденные шрифты он кладёт рядом в `fonts/` под чистыми латинскими именами (`CharterITC-Bold.otf`) — ровно под теми, что записаны в `template.json`. Формат номера скрипт выводит сам («(1)» → «({N})»). Формат даты —
...
## Номер в кружке и водяной номер

- **Номер в кружке.** В макетах ОРС номер вида «(14)» шрифт Fedra Sans Pro
  рисует OpenType-функцией: закрашенный круг, цифры вырезаны насквозь. GD
  OpenType-функций не применяет, поэтому скрипт узнаёт такой слой (текст
  «(цифры)», рамка квадратная) и пишет в spec `"badge": {"shape":"circle",
  "diameter": N}`, формат `{N}`, кегль цифр 0,62 диаметра — рендер рисует круг
  сам. У обзорного занятия кружок не рисуется.
- **Водяной номер внутри стопки слоёв** (Кочергина: огромная «40» над плашками,
  но под фото, «Мягкий свет», заливка 59 %): `--watermark-layer "40"`. Скрипт
  делит макет на `background.png` (слои под водяным номером) и прозрачный
  `overlay.png` (всё выше), поле идёт в spec с `"layer":"under"`,
  `"blend":"soft_light"`, `"opacity"`. `overlay.png` грузится в «Верхний слой»
  той же формы; без него шаблон с таким полем не примется.
