_Created: 23-09-2026 · Last updated: 24-09-2026 (H5397: band pass blocked on the Mac keychain — GUI session needed; measured `offline_misses: 865`)_
...
## Band pass, second attempt — 24-09-2026 (H5397): blocked on the keychain, not on the rate limit

The residual m1 was minted as H5397 and picked up by an unattended Windows worker. The pass did **not** start: the band lookups need the `ruscorpora-api` token, and no box an agent can drive holds a readable copy of it.

Measured state on `main` at 2c3f5d94, `python scripts/nkrya_gloss_lint.py lemmas roots --offline`:
...
865 remaining lookups at roughly 60 an hour is about **14.5 hours** — unchanged from the 23-09 estimate, and the acceptance bar (`offline_misses: 0`) is still that one long unattended run away. The lint TSVs regenerated byte-identical, so nothing has drifted since H5285.

What was probed on 24-09-2026, each live, in this order:

1. Windows box (MSI): `RUSCORPORA_API_TOKEN` unset; `keyring.get_password('ruscorpora-api', 'token')` returns `None`; `storage/app/nkrya_cache` is empty. No token here.
2. The Mac is up on the tailnet (`macbook-air`, 100.90.97.31) and accepts key-auth ssh as `mac`. Both checkouts are present; `/opt/homebrew/bin/python3.13` is the only python there with `pymorphy3` (system `python3` has none).
3. The keychain **item exists** — `security find-generic-password -s ruscorpora-api` prints its attributes, created 20260923073914Z, `acct="mac"`.
4. Reading its **value** over ssh fails: `security … -w` exits **36** (`errSecInteractionNotAllowed`), and `security show-keychain-info` says `User interaction is not allowed`. The login keychain does not open for a non-GUI session.
5. The usual workaround does not apply either: `launchctl asuser $(id -u) security …` answers `Could not switch to audit session: Operation not permitted` (it needs root), and `sudo -n true` on the Mac needs a password.

So this is a human-gated step, and the gate is a GUI session — not a missing credential, not the rate limit. A Terminal window opened on the Mac itself inherits the unlocked login keychain and reads the token with no prompt.

[`scripts/nkrya_h5397_mac_launch.sh`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/scripts/nkrya_h5397_mac_launch.sh) does the whole launch in one line: it refuses if a run is already live (`pgrep`), reads the token, fetches `origin`, creates the worktree `Systema-Sanscriticum-h5397-mac` off `origin/main` (the pass writes the tracked evidence cache, so it may not run in the guarded main tree), and `nohup`s the run into `~/nkrya_h5397.log`. On the Mac:

```bash
bash ~/Documents/GitHub/Systema-Sanscriticum/scripts/nkrya_h5397_mac_launch.sh
```

Afterwards ssh is fine again — the run survives the launching terminal, and progress is `tail -f ~/nkrya_h5397.log`.
