...
## 7. Уборка осиротевших эмбеддингов (после пере-экспорта FAQ)

`knowledge:index` умеет только upsert, а `chunk_id` выводится из заголовков
разделов: когда ORS-FAQ пере-экспортирует `faq.md` с переименованными
заголовками, старые строки остаются в `knowledge_chunks` навсегда. Замер на
проде 18-09-2026: **82 чанка корпуса против 159 строк FAQ-полосы — 77 сирот**
(~308 КБ BLOB'ов). В ответы сироты не попадают (ретривал идёт по чанкам
корпуса), но плотная нога читает их из базы на каждом запросе, а
`knowledge:coverage` показывает раздутый `rows=`.

Проверить и убрать:

```bash
php artisan knowledge:index                   # сирот только считает и печатает, ничего не удаляя
php artisan knowledge:index --prune-orphans   # удаляет их (только FAQ-полоса своей модели)
php artisan knowledge:coverage                # rows должно сойтись с chunks
```

Удаляет только FAQ-полосу своей модели/размерности: строки уроков и строки
другой модели не трогаются. Без `--prune-orphans` команда сирот считает и
печатает, но не удаляет; при пустом корпусе удаление невозможно вообще (пустой
парс = отказ команды). Первый прогон на проде — человеком и осознанно: это
удаление строк, а не индексация.
