...
### Ручные напоминания — закрыто H3156 (19-08-2026)

До H3156 кнопка «Напомнить в TG/VK» на «Должниках» следа не оставляла, и такой
контакт правилу был не виден. Теперь все три ручных пути пишут строку
`debt_reminders` с `source = 'manual'`: одиночное действие и массовая рассылка
на «Должниках» и напоминание по обещанию в «Очереди работ».

Ключевая осторожность: та же таблица обслуживает **анти-спам** лестницы дожима.
Наивная вставка строки чинила бы правило и молча глушила следующее
авто-напоминание на весь `debt_reminder_cadence_days`, отодвигая вместе с ним
эскалацию `DunningStage`. Поэтому источник различается, а политика вынесена
человеку: тумблер **«Ручное напоминание глушит следующее авто»** в настройках
маркетинга (`debt_reminder_manual_suppresses_auto`), **по умолчанию выключен** —
то есть ритм лестницы остаётся ровно таким, каким был до H3156. Для правила
H2746 контакт есть контакт: считаются обе разновидности, различие уходит в
аудит-след (`contact_attempts[].channel`).

Исторические строки помечены `auto` не для удобства, а по факту: до H3156
`DebtReminder::create` вызывался ровно в одном месте — в авто-команде.
...
молчит. Второе по весу — контакты: авто-лестница включена
(`debt_reminders_enabled = 1`), но на конкретную пару (студент, курс) двух
записанных обращений почти нигде ещё не накопилось. Ручные напоминания на
момент прогона следа не оставляли — это закрыто в тот же день (§2, H3156), и
счёт контактов начнёт расти с первой ручной отправки после выкладки.

Практический вывод: до массовой привязки `telegram_id` контур будет
отчитываться пустым списком даже теперь, когда ручные напоминания логируются
(H3156). Это **правильное** поведение — исключать человека из чата на основании
ненаписанного нельзя, — но означает, что связывающим ограничением остаётся
именно `telegram_id`, а не механика правила.
