...
## Волна 3 — починка сбора посещаемости (отдельно) · ✅ отгружена и применена на проде (H3761, 31-08-2026)

**Результат на проде (`39b3535c`, смоук 200):** первый поток поднят из Zoom — заведено
**22 занятия** 17-09-2025…18-03-2026 (20 сред + 2 четверга, H3771) и вставлено
**563 строки** посещаемости; повторный прогон даёт 0/0 (идемпотентность проверена на
боевых данных). Плашка покрытия по семье 332/375/424: **было 17 из 70 (23,9 %), стало
30 из 70 (42,9 %)**, узнано по вебинарам **было 0, стало 20**. У курса 375 досбирать
было нечего — 0 строк к вставке, что и предсказывал диагноз. Ни одна существующая
строка не изменена и не удалена.

**Ловушка слота (H3771):** четверговое занятие 01-01-2026 началось в **12:29**, на
минуту раньше средового окна `12:30`, и узкий `--slot` потерял бы его молча. Граница
окна — наблюдение, а не расписание: сверяйте её с фактическим временем запусков Zoom.
...
4. **Четверги первого потока — «да, четверги тоже заводи» (H3771).** Два занятия (01-01-2026 и 12-03-2026) добраны: 2 занятия, 34 строки. Окно расширено до `Thu,12:00-15:30` — занятие 01-01-2026 началось в 12:29, на минуту раньше средовой границы, и узкий слот потерял бы его молча.
5. **Нумерация уроков против числа встреч — «оставить как есть».** 22 проведённых занятия и 16 уроков «1/16»…«16/16» описывают разное: занятие — факт живой встречи, к которой цепляется посещаемость; урок — единица материала в кабинете. Переименование сломало бы ссылки на уроки и разошлось бы с курсом 424 («в записи»), где нумерация та же. Расхождение — не дефект и не дубли: колонка занятий считает встречи, колонка уроков — материалы.

6. **Ручная связка подписей — «nobody will link them manually, maybe an agent with fuzzy?» (H3772).** Верно: план «куратор свяжет вручную» не выполняется никогда, а перепись показала, что устройствами и телефонами были лишь **5 подписей из 96** — остальные 91 живые имена, которые ломала транслит-таблица (`Olga`→олга против ольга, `Yulia`→юлиа против юлия, `Sergey`→сергеы против сергей). Добавлены складка алфавита, уменьшительные и расстояние Левенштейна по символам ([`ZoomNameMatcher`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Support/ZoomNameMatcher.php)); ничья, одинокое слово и слово с опечаткой по-прежнему отказ. На проде — 12 связок, все проверены глазами перед записью и все верные, повторный прогон 0.

**Волна 3 закрыта.** Плашка покрытия по семье 332/375/424 прошла путь **17 из 70 (23,9 %) → 30 из 70 (42,9 %) → 35 из 70 (50,0 %)**; узнано по вебинарам **0 → 20 → 26**.

Остались подписи, которые **нечем** опознать, а не те, до которых не дошли руки: имена устройств, телефоны, односложные ники и настоящие неоднозначности (несколько Анастасий среди плательщиков одного курса). Алгоритм здесь не поможет — признака нет. Дальше цифру двигает профилактика, а не разбор старого: просьба входить в Zoom под своим именем уже записана в руководства [ученика](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/docs/STUDENT_CABINET_GUIDE_RU.md) и [куратора](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/docs/CURATOR_ADMIN_GUIDE_RU.md).
