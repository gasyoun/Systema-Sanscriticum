...
| API-канал Метрики | токен в `~/.secrets/metrika.env` **рабочий** (200, аккаунт `gasyoun`, проба 18-09 ~17:05; разовый 401 в ~16:52 не воспроизвёлся). Счётчик `106964341`: **22 цели** — ручные `lead`, `telegram_click`, URL-воронка («Открыл карточку курса» / «Начал оформление (checkout)» / «Завершил заказ (thank-you)»), CRM-цели (order created/paid/spam/cancelled), 9 автоцелей, 5 URL-целей промо-страниц. **Разрыв:** ни одного из 6 канонических имён `funnel_events` — `reachGoal` из кода сейчас не ловится ни одной целью; кабинетных целей нет | Uprava [tools/pull_metrika_weekly.py](https://github.com/gasyoun/Uprava/blob/main/tools/pull_metrika_weekly.py) · management API census 18-09-2026 |
