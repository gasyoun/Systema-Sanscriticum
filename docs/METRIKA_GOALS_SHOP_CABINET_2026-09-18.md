...
| API-канал Метрики | токен в `~/.secrets/metrika.env` рабочий (200, аккаунт `gasyoun`; разовый 401 ~16:52 не воспроизвёлся). **✅ 18-09 ~17:20 — 16 целей созданы через management API по рулингу MG «заводи»** (ids 639422183–639422234: 6 канонических + 10 кабинетных Tier 1), счётчик 22 → **38 целей**, контрольное чтение — ноль пропусков. Схема POST — обёртка `{"goal": …}` с условием `{"type":"exact","url":<идентификатор>}` (прямой `{"type":"action","value":…}` даёт 400 «must not be null»). Канонические 6 считают сразу (reachGoal уже в коде); кабинетные оживают с посадкой моста (§Implementation 1–4). Прочие цели счётчика: ручные `lead`, `telegram_click`, URL-воронка, CRM-цели, 9 автоцелей | Uprava [tools/pull_metrika_weekly.py](https://github.com/gasyoun/Uprava/blob/main/tools/pull_metrika_weekly.py) · management API census 18-09-2026 |
