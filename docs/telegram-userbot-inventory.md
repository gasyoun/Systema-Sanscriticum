...
### 4.3. Почему Track C ушёл с вебхука на long polling (27.07.2026)

Продолжение §4.2, пункт 3: сообщения бот-чата пропали 22.07. Разбор **по проду**,
а не по догадкам:

- вебхук был зарегистрирован на `https://samskrte.ru:88/...`, порт 88 на новом
  сервере не слушается; после перерегистрации на 443 ошибка **не изменилась** —
  `Connection timed out`;
- в nginx-логах **ноль** запросов с подсетей Telegram (`149.154.*`, `91.108.*`)
  за всю историю. Все вебхуки приходили с `10.9.0.1` — приватного адреса
  туннеля — и оборвались 22.07 в 19:01 UTC;
- при этом сайт снаружи открывается, а вебхуки Точки доходят (27.07, 11:06 и
  11:45, оба `200`) — то есть входящие в принципе работают, не проходят именно
  соединения из подсетей Telegram;
- туннеля, дававшего `10.9.0.1`, на сервере не осталось: из служб живёт только
  `tg-tunnel.service` — sshuttle, заворачивающий подсети Telegram **на выход**.

Вывод: входящего канала для вебхука нет, а исходящий уже построен и работает.
Поэтому Track C принимает апдейты **long polling'ом** — `zapisi:poll`
({@see App\Console\Commands\PollTelegramZapisiUpdates}) ходит `getUpdates` по
sshuttle-туннелю и отдаёт апдейты в тот же `ProcessTelegramZapisiUpdate`, что и
вебхук: формат `update` идентичен, дорожка хранения одна.

**Вебхук и поллинг взаимоисключающи** — пока вебхук зарегистрирован, `getUpdates`
отвечает `409 Conflict`. Команда снимает его сама при старте (однократно, с
записью в лог). Обратный переход — `zapisi:set-webhook`, но он имеет смысл только
если входящий канал восстановят.

Демон живёт под supervisor (`deploy/supervisor/zapisi-poll.conf` — конфиг в репо,
не на сервере), планово выходит раз в `poll_max_lifetime_seconds` и
перезапускается `deploy.sh` вместе с Horizon: долгоживущий процесс иначе держит
старый код после выката — та же грабля, что уже описана в скрипте про Horizon.
