...
_Created: 01-07-2026 · Last updated: 29-07-2026_
...
| ~~Topics web-chat-side~~ | 4 | **✅ Closed 29-07-2026 (H1837).** [`SupportTopicClassifier`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Services/TelegramSupport/SupportTopicClassifier.php) now takes the day's text from whichever store the rollup's `channel` names, so the **same** `SupportTopicRule` set classifies web threads too. One rule set on purpose — a second web-side taxonomy would give one topic two categories depending on where the student wrote. |
...
| ~~Web-chat analytics missing~~ | 5 | **✅ Measurement closed 29-07-2026 (H1837)** — see *Deflection measurement is cross-channel* below. `support_daily_rollups` carries `channel`; web threads are rolled up by [`WebChatDailyRollupAggregator`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Services/Support/WebChatDailyRollupAggregator.php) and counted by `support:topic-ranking` / `content:detect-gaps`. The **operator dashboard** [`TelegramSupportAnalytics`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Filament/Pages/TelegramSupportAnalytics.php) stays Telegram-only by design (it renders TG transcripts) — a web-side dashboard page is still open. |
...
## Deflection measurement is cross-channel (H1837, 29-07-2026)

`support_daily_rollups` used to describe exactly one channel — `telegram_support_chat_id` was NOT NULL — so everything computing deflection on top of it measured Telegram alone. A topic arriving mostly through the site widget scored as if barely anyone asked it, which **biased the very ranking that decides what to automate first**. That was a measurement bias, not a missing screen.

- A rollup row now carries **`channel`** (`telegram` | `web`) plus a nullable `support_conversation_id`; exactly one of the two FKs is set. Existing rows default to `telegram`, so the Telegram numbers are byte-identical to before.
- Web rows are produced by [`WebChatDailyRollupAggregator`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Services/Support/WebChatDailyRollupAggregator.php), the twin of [`SupportDailyRollupAggregator`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Services/TelegramSupport/SupportDailyRollupAggregator.php). Direction / human-vs-AI is **not** re-derived there: it goes through [`UnifiedMessage::fromChatMessage()`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Support/UnifiedMessage.php), the single source of truth for the web role mapping. Because that mapping is `source`-blind, VK and TG-student-bot threads (`chat_messages.source`, H1200) roll up as `web` too.
- Trigger differs by channel: Telegram is rolled up by the sync itself (affected dates); the web side has no sync, so `support:rollup-web --days=2` runs nightly at 04:35 ([`Kernel`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Console/Kernel.php)). Idempotent per (thread, day) — safe to re-run and to backfill with `--date=`/`--days=`.
- **Consumers split deliberately.** Cross-channel: `support:topic-ranking` (with `--channel=all|telegram|web` and a `web %` column showing where a topic arrives) and `content:detect-gaps`. Telegram-scoped via the `->telegram()` scope: `SupportDashboardPacketBuilder`, `TelegramSupportAnalytics`, `SupportObservability` — they read TG transcripts and would fatal on `$rollup->chat->…` for a web row. **Any new consumer must pick a side explicitly.**
- **Both channels on one screen:** [`SupportObservability`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Filament/Pages/SupportObservability.php) now renders a second "Веб-чат" card with the same five numbers for `channel=web`, next to — not merged with — the Telegram card. Merging them would misread: the page's neighbouring cards (userbot sessions, reply delivery) are Telegram-only, so a combined total would be taken for a userbot metric.
- **Flag:** the one new write path (the nightly command) sits behind `features.web_chat_deflection_rollup`, **OFF by default**. While it is off no `web` row is ever written and every report returns exactly its pre-H1837 numbers. Turning it on: `WEB_CHAT_DEFLECTION_ROLLUP=true` + `config:cache`, then one backfill pass `support:rollup-web --days=N --force`. The read side is deliberately **not** flagged — it counts whatever is in the table, and a second switch would create two states that can disagree.
- Messages with no `support_conversation_id` (pre-H536 rows) are not rolled up: there is no grouping key, and inventing a thread from `user_id` after the fact would be fabricating data.
