...
| `H3952_INFRASTRUCTURE_FAILURE` | HEAD вебхук-токена сам не получил HTTP-ответа (status=0: транспорт/краш ноды, или 5xx) — упала проверка/инфраструктура бокса, а НЕ токен и НЕ cred. Прецедент: exec 2749, ENOSPC диск .91 — до H4513 (закрыт 13-09) else-ветка ложно называла это `H3952_WEBHOOK_MISSING` | освободить диск/проверить сеть .91 (§6.1) и повторить прогон; класс «записи нет» и «сбой cred» НЕ подтверждён |
...
«Обложка найдена?» ищет `name = '<дата старта>.jpg'` в папке курса (drive_folder_id из листа). Пусто → «Обложки нет — пропуск» → Merge (mode=combine; вход 0 приходит только с ветки обложки: `Add a playlist item (Hindi)` → `ЗАГРУЗКА НА РУТУБ`) → Merge не срабатывает → n8n завершает exec SUCCESS, молча пропустив `ЗАГРУЗКА НА РУТУБ1`/плейлист/Rutube2/субтитры/DeepSeek/`СОЗДАЁМ УРОК В АДМИНКЕ1`/финальный TG. Симптом: exec `success` за минуты, в runData < ~30 нод, последний узел Merge; при этом YouTube уже залит — полный реплей вебхука = дубль (`Upload a video (Hindi)` без дедупа). Прецеденты полного пути: exec 2723 (64 ноды), 2356 (59 нод); мёртвый пример — 2754 (28 нод). Разбор: [INCIDENT_N8N_ENOSPC_HINDI_COVER_MERGE_DEADEND_10-09-2026.md](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/docs/INCIDENT_N8N_ENOSPC_HINDI_COVER_MERGE_DEADEND_10-09-2026.md). Ремонт графа — H4513, ЗАКРЫТ 13-09-2026 (бэкап `/root/wf_backup_pre_h4513.json`): ребро «Обложки нет — пропуск» ПЕРЕНЕСЕНО с Merge (вход 1) на «ЗАГРУЗКА НА РУТУБ» (вход 0) — MOVE, не второе параллельное (skip→Merge рёбер после правки ноль, GET-проверено). Семантика: Merge стреляет по входу 0 (2356 прошёл с 2 из 3 входов), а вход 0 «ЗАГРУЗКИ НА РУТУБ» питался ТОЛЬКО цепочкой обложки — без обложки прогон умирал зелёным на Merge. Теперь skip-ветка кормит Rutube-загрузку напрямую (её выражения ссылаются только на именованные ноды, item `{cover_skipped:true}` ей безвреден), и хвост доезжает без обложки; развилка IF взаимоисключающая — дублей загрузки нет. Deploy-механизм: `scripts/n8n_zoom14_h3952_freshlink_verdicts.py` §6 (идемпотентный точечный PUT). Осторожность: до первого живого no-cover прогона класс считается закрытым по топологии + форс-тесту вердикт-нод, не полным реплеем (запрещён — дубль YT).
