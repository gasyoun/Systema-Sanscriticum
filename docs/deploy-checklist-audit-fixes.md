...
Отдельно: Zoom-посещаемость требует ещё Event Subscription, report-scope и
...
## Что ещё едет в этом merge (для контекста)
...
- Соцавторизация: привязка по email только при подтверждённом email провайдера
