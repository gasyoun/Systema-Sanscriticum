...
Severity в `guards:verify` зависит от исхода: метка `[rolled-back]` в причине
(сайт восстановлен откатом) — **warning**; без нее (прод может быть нездоров,
или откат не помог) — **critical** → `cabinet:probe` шлет тревогу в Telegram
