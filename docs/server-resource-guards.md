...
случае — обёртка снимает заход по `timeout` на `SCHEDULE_MAX_SECONDS`, а
пережившего это straggler'а добивает reaper при возрасте > 2x потолка:
...
1. **Замок снимается `schedule:finish`, которого при убийстве не будет.** Убийство
   — теперь ШТАТНЫЙ путь завершения (watchdog, `timeout` обёртки, reaper). Фоновый
...
- Если `flock -n` проваливается и возраст файла превышает
  ...
  заведомо позже собственного порога reaper'а на добивание застрявших `artisan`),
...
поэтому синтаксис проверен `bash -n`, а сам прогон — на Linux до раскладки на прод).
...
> **Ловушка выкладки: правка ЛЮБОГО managed-файла роняет СЛЕДУЮЩИЙ деплой.**
> Наступили на неё при выкатке самого H2149 (02-08-2026). Порядок такой:
> `deploy.sh` в конце зовёт `guards:verify`, а тот сверяет
> `/usr/local/sbin/systema-auto-deploy-run.sh` с копией в репозитории. Сразу после
> merge копии РАЗНЫЕ (в git — новая, на машине — старая), это честный drift →
> `guards:verify` недоволен → `deploy.sh` выходит 1 → обёртка видит провал и
> **откатывает деплой**, унося ровно тот коммит, из которого `apply.sh` и должен
> был поставить новую версию. Снаружи выглядит как «деплой не идёт», хотя всё
> отработало по замыслу. Правильная последовательность:
>
> ```sh
> cd /var/www/html && bash deploy.sh        # код доехал; guards:verify ругнётся на drift — это ОЖИДАЕМО
> bash scripts/server_guards_apply.sh       # ставит новый managed-файл, drift исчезает
> sudo -u www-data php artisan guards:verify  # «все проверки пройдены»
> ```
>
> Это относится к любой строке `manifest.psv`, а не только к обёртке
> авто-деплоя: правка managed-файла — всегда двухшаговая выкладка.
