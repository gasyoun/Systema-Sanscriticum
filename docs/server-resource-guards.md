...
_Created: 29-07-2026 · Last updated: 16-09-2026_
...
### 12.4 Второй класс отказа: медленный канал × большой backlog (07-09-2026,
### H4306)

Первый реальный прогон юнита под нагрузкой вскрыл класс, под который 1200 с
не sizing'ился. Ночной архив `2026-09-07-02-02-19` = 126 частей × 20 МиБ;
Yandex WebDAV отдавал ~112 КБ/с (187.3 с на часть, стабильно, все 39 PUT).
1200-секундное окно вмещает 6 частей × 187 с = 1122 с — юнит убивался
посреди 7-й части КАЖДЫЙ час: 6 таймаутов подряд с 00:10 UTC, failed-статус
тригерил `guards/failed-units` (soft) каждые 10 минут, TG soft-sticky —
при том, что прогресс шёл в КАЖДОМ окне (аудит по часам local: 02ч×3,
03–08ч×6, нулевых окон нет — hang-класс 24-08 не повторялся).

Различение классов: hang = PUT висит БЕЗ прогресса (лечится таймаутом как
страховкой); throughput = PUT-ы завершаются, их больше, чем влезает в окно
(лечится окном под полный backlog). `YANDEX_RESUME_TIMEOUT_SECONDS`
1200→28800: полный дренаж 6.55 ч + запас ~20 %. Повторный тик таймера при
активном oneshot-прогоне — no-op, дублей не плодит; во время длинного
прогона юнит active (running), не failed — failed-units guard молчит.

Residual (не блокирует): `CURLOPT_TIMEOUT=300` на часть при наблюдаемых
187 с — запас 60 %; падение throughput на ~40 % начнёт убивать PUT-ы по
curl-таймауту → бесконечные re-PUT. Кандидат: поднять до 600 или считать
от размера части. Отдельный класс-кандидат: при повторном листинге части,
загруженные в ПРЕДЫДУЩЕМ окне, но ещё не видимые (лаг H3371), re-PUT'ятся
заново — двойная работа на медленном канале.

## 13. guards/cgroup: заполненность ≠ давление (PSI-гейт, 25-08-2026)

В тот же день, когда waiver погасил tmpfs-cap, вскрылся второй источник того же
шума: предупреждение `guards/cgroup` «занял N из 2048 МиБ (≥80 %)» флапало
вокруг порога — 1547→1855 МиБ за час, каждое пересечение давало деплою красный
блок exit 75 и меняло soft-fingerprint (повторные Telegram поверх sticky).

Замер .92 того же дня объяснил почему: memory.stat группы = **anon 63 МиБ +
file cache 1.5 ГиБ**. «Память» почти целиком page cache от GB-scale IO кронских
детей, который ядро вытесняет само без чьего-либо ведома; мгновенная
заполненность high — не давление.

Гейт: предупреждение ≥80 % теперь требует, чтобы ядро САМО подтвердило
троттлинг — `memory.pressure` группы (PSI) `some avg10 ≥ CGROUP_PSI_SOME_PCT`
(1.0 %). Правила:

| Случай | Поведение |
|---|---|
| ≥80 % заполнения, PSI ниже порога | молчим — чистый кеш |
| ≥80 % + PSI some avg10 ≥ порога | warning с числом PSI в тексте |
| файл PSI не читается | старое поведение без гейта (fail-open) |
| current ≥ memory.high | critical БЕЗ гейта — ядро уже на пределе |
...
## 15. Владение в `storage/`: осознанные root-исключения (0A64, 15-09-2026)

Перепись 15-09-2026 (`find storage -type f ! -user www-data`, остаток верификации
H4848) нашла 543 файла не-`www-data`; 532 переведены на `www-data:www-data`
([changelog](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/changelog_queue/2026-09-15-h4848-residuals-deploy-rerun-storage-chown.md)).
Три класса оставлены root **сознательно** — будущая перепись должна ожидать
именно их, всё остальное = новый root-писатель, разбирать:

| Класс | Файлов | Почему root — это норма |
|---|---|---|
| `storage/app/server_guards/` | 3 | зеркало root-крона H1941 + бейслайны гардов: пишет сама обёртка/apply (`chown root:www-data` — по конструкции) |
| `storage/framework/auto-deploy.lock` | 1 | lock-файл обёртки авто-деплоя, пересоздаётся root-кроном при каждом прогоне |
| `storage/app/telegram-support/marcisgasuns/session.madeline/` | 7 | **принято MG 16-09-2026** («accept + document»): воркер личной Telegram-сессии запускается вручную от root — это осознанный выбор, а не дефект |

Про третий класс, чтобы вопрос не всплывал снова: воркер `marcisgasuns` держит
сессию личного аккаунта, стартован руками от root (проверено 16-09-2026: PID
1502338, PPID 1, uptime >1.5 сут; ни unit, ни supervisor, ни cron на него не
ссылаются — ручной запуск и есть «стартер»). Две родственные сессии
(`telegram-support`, `rusamskrtam`) живут как www-data под
`systema-madeline-daemon.service` (§9) — `marcisgasuns` единственное исключение.
`chown` под живого root-писателя бесполезен (файлы пересоздаются), поэтому
класс не «чинится», а документируется здесь. Если воркер умрёт — перезапустить
вручную тем же способом, каким он был запущен; автоматизировать (перевод под
www-data) можно в любой момент, решение за MG, ряд 0A71 закрыт этим выбором.
