...
Что при этом **работает и уже стоит**: правило старения `tmpfiles.d`
(`systemd-tmpfiles` не зависит от того, кто монтировал) и проверка `tmpfs-cap`,
которая остаётся `critical` — опасность-то никуда не делась — но в этом случае
называет хост, а не `server_guards_apply.sh`. Критическая тревога, посылающая
человека к скрипту, который заведомо бессилен, хуже отсутствия тревоги.
