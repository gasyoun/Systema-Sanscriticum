...
_Created: 05-07-2026 · Last updated: 07-08-2026_
...
## Решения (закрыты 07-08-2026, чат)

Все три бывшие `@DECIDE` закрыты в сторону рекомендаций спеки. Реализация Phase 1
идёт по этим правилам без повторного опроса.

1. **Куда встаёт диагностика — блок на закрытом уроке + сводка в профиле.**
   «Почему закрыто?» на карточке закрытого урока (контекст под рукой) + краткая
   плитка статуса в секции ботов/профиля. Отдельная вкладка «Мой доступ» **не**
   обязательна в Phase 1 (можно добавить позже без смены диагноста).

2. **Авто-самолечение без куратора — ровно три действия.**
   - «Открыть оплаченные блоки» → `BlockAccessMaterializer` (идемпотентно);
   - «Подключить бота» → deep-link `telegram.connect` (и VK-аналог, если уже есть);
   - «Сбросить пароль» / magic-link → `password.request` (и существующий magic-link path).
   Всё, что меняет деньги/долг — только через долговое самообслуживание.
   Смена email/телефона студентом — **нет** (остаётся куратору).
   Иначе — кнопка «позвать куратора» (штатный хендовер кабинет-бота).

3. **«Оплатил, а доступа нет» без покрывающего платежа.**
   - есть `pending` / `failed` на этот блок → «проверить статус оплаты»;
   - платежа нет → вести в долговое самообслуживание («Мои долги» / checkout);
   - **не** выдавать доступ «по слову» и не auto-grant без оплаченной строки в LMS.
