...
|---|---|---|---|---|---|
| 2026-08-21 07:06 | host/ops body walk: yandex 11 MiB + «no off-site» | HTTP 200. php-fpm/nginx active. `/` 23/147 GiB. Spatie `local` zip `storage/app/Laravel/2026-08-10-02-01-48.zip` 1.4 GiB (`newestBytes` 1425272697). Spatie `yandex_disk` reachable, `newestBytes` 11757968 (~11 MiB), fresh mtime — stub, `backup:monitor` still Healthy. Monday 17-08 `backup:run` died in 1 s: `telegram-harvest/pilot/manifests` Permission denied (dir now www-data 775). **Do not** run the TG SOS runbook (`cabinet:probe` from SSH) for this class. Live off-site is restic → `.91`: last 07:04:42Z `systema=OK samudra=OK` snapshot `80bd92a7` / `26a2d064`. `backup-fresh` does not see restic. HEAD `2ffebce5`. | **No code.** Do not `backup:run` into the fat cron cgroup today. Next Spatie weekly: Monday 24-08 02:00. Artem not paged. | Grok 4.6 (`grok-4.6`) | class: Spatie/Yandex stub vs live restic |
