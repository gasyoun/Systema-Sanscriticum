...
1. Написать `tools/lint_pedagogy_model.py` с проверками L1–L6, прогнать по YAML, починить всё, что всплывёт.
2. Выполнить P0.1–P0.4 (только чтение, агрегаты, без выгрузки персональных данных).
3. Записать результаты P0 в таблицу VERIFICATION; всё, на что данные не отвечают, — отдельной строкой как GAP.
4. Пересмотреть слой следов в YAML по фактическому положению дел; убрать `trace`, который не подтвердился.
5. Обновить журнал ревизий в метадоке модели.
