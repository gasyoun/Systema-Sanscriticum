...
1. ⬜ Написать `tools/lint_pedagogy_model.py` с проверками L1–L6, прогнать по YAML, починить всё, что всплывёт. **Осталось.**
...
подтверждено. **Пункты 2–5 закрыты; пункт 1 остаётся — то есть W1 не завершён.**
