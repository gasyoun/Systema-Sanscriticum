...
| **Членство / клуб** | **NO-GO** | Полка записей пуста: купивший клуб за ₽1 500 не получает ничего |
| **Деньги/доступ** | **не тронуты** | Все три флага OFF, `club_memberships=0`, `free_tier_grants=0` |

**Единственный блокер запуска клуба — шлюз C3 (полка записей).** Это решение
человека, а не работа агента: агент не вправе назначать, какие записи входят в
подписку за ₽1 500, когда тот же курс живьём стоит ₽6 000.
...
| C3 | **Полка записей** | **NO-GO — ЕДИНСТВЕННЫЙ БЛОКЕР** | `membership:rehearse` шаг 3 = FAIL: ни одного курса с `club_included=true`. `Course::where(club_included,true)->count()` = **0** | **MG (решение человека)** | **§5 шаг 1** |
...
| C7 | Сквозная репетиция `--apply` | **N/A — уперлась в C3 и в ограждение** | Без полки шаг 3 = FAIL, и команда сама отказывается идти дальше («предпосылки не выполнены — прогон не начат»). Кроме того `--apply` создаёт строку `Payment` — агенту это запрещено ограждением H2865 | MG | §5 шаг 2 после шага 1 |
