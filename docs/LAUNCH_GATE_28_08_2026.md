...
| **Деньги/доступ** | изменены **два** флага, оба по указанию MG | `club_membership=true`, `membership_cancellation=true`; **`membership_free_tier` остаётся OFF** (раздаёт гранты 350 уснувшим — по порядку идёт последним, после живых оплат). Ничего не создано: членств 0, платежей по курсу 444 — 0, `free_tier_grants=0` |
...
`MEMBERSHIP_CANCELLATION` тоже включён 16-08 (§5 шаг 3). Осталось человеку:
**один живой чекаут** клубного тарифа — единственное, что проверяет денежный
путь до конца, — и `MEMBERSHIP_FREE_TIER` **последним** (§5 шаг 4), после этого
чекаута. Порядок не косметический: бесплатный уровень раздаёт гранты 350 уснувшим
плательщикам, и включать его до подтверждения, что клубная оплата вообще
доходит, значит раздать доступ на непроверенном контуре.
