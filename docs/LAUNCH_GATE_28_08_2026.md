...
| **Деньги/доступ** | включены **все три** флага, каждый по указанию MG | `club_membership` · `membership_cancellation` · `membership_free_tier` — все `true` (16-08). Бесплатный уровень пошёл **не** авто-подбором: демон раздаёт только когорту из файла ([H2915](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2915-Opus_Systema-Sanscriticum_free-tier-daemon-reads-cohort-file_16.08.26.md)), выдано **149** грантов по явному списку молчащих >12 мес, метка `free_tier_h2566`, до 15-09. Клубных членств 0, платежей по курсу 444 — 0 |
...
`MEMBERSHIP_CANCELLATION` и `MEMBERSHIP_FREE_TIER` тоже включены 16-08 (§5 шаги
3–4) — оба по прямому указанию MG.

**Порядок из очереди деплоя нарушен, и это стоит знать.** Он ставил бесплатный
уровень последним, **после** живого чекаута, подтверждающего денежный путь;
чекаута не было и нет — платежей по курсу 444 ноль. Риск при этом снят не
порядком, а механикой: демон не раздаёт авто-подбором (H2915), выдача пошла
явным списком на 149 человек, и клубная оплата тут ни при чём. Остаётся
человеку по-прежнему **один живой чекаут** клубного тарифа — он единственный
проверяет денежный путь до конца, и до него контур «оплата → членство» не
подтверждён ничем, кроме репетиции.
...
| C5 | Отмена подписки | **PASS — ВКЛЮЧЕНА 16-08-2026** | `MEMBERSHIP_CANCELLATION=true` по указанию MG. `POST membership/cancel` и `POST membership/resume` появились в `route:list` — при выключенном флаге их там не было; оба под авторизацией, гостю не видны. `MembershipCancellationTest` 8/8 зелён, включая «отменивший сохраняет полку до конца оплаченного периода» | — | откат: убрать строку + `config:cache` |
| C6 | Бесплатный уровень | **PASS — включён и проверен на живом кроне 17-08-2026** | `MEMBERSHIP_FREE_TIER=true` (16-08, MG). Первый настоящий крон-прогон: **05:25:42 17-08, DONE за 1 с**, тревог 0. Грантов `free_tier*` **149 → 149** — ноль новых, как и предсказывалось до прогона (у 84 из первых 200 право живо до 15-09, у 116 нечего открывать). Источник подтверждён отдельно: живой прогон **от `www-data`** печатает `демон/файл`, `cohort_file=membership/free_tier_cohort.txt`, `daemon_apply=false`. Один счётчик тут ничего не доказывает — «сработал по файлу», «свалился на авто и был заглушён» и «не запускался» дают одинаковый ноль; различают источник в выводе и код возврата DONE, который исключает ветку фейл-клоузд ([H2933](https://github.com/gasyoun/Uprava/blob/main/handoffs/H2933-Sonnet_Systema-Sanscriticum_free-tier-cohort-daemon-first-run-verify_16.08.26.md)) | — | первая настоящая выдача — середина сентября, по мере истечения 30-дневных грантов |
...
| C8 | Денежные флаги | **PASS — два включены по указанию MG, третий НЕ тронут** | `club_membership=true` (11:52 MSK) и `membership_cancellation=true` (12:26 MSK), оба 16-08-2026 по прямому указанию MG. **`membership_free_tier=false` — не трогали**: он выдаёт месячные гранты 350 уснувшим плательщикам, и порядок ставит его последним, после проверки клуба на живых оплатах. Ничего не создано: `club_memberships=0`, платежей по курсу 444 — **0**, `free_tier_grants=0`. Бэкапы прод-`.env`: `.env.bak-h2886-20260816-115203`, `.env.bak-cancel-20260816-122626` | — | откат любого: убрать строку из `.env` + `php artisan config:cache` |
...
**Шаг 3 — ✅ ВЫПОЛНЕНО 16-08-2026 по указанию MG.** Повторять не нужно.

```bash
# в .env прода (строка 357):
MEMBERSHIP_CANCELLATION=true
php artisan config:cache   # НЕ config:clear — см. шаг 2
```

Проверка, которая отличает «маршрут есть» от «маршрута нет»:
`php artisan route:list | grep membership` — должны быть `POST membership/cancel`
и `POST membership/resume`. При выключенном флаге их в списке нет вовсе (это и
есть тест «routes are 404 while the flag is off»). Оба POST и под авторизацией —
гостю ничего не видно.
