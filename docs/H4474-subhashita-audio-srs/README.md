...
- [WIRING_PLAN.md](WIRING_PLAN.md) — design-only plan to extend the `kosha-srs-deck-b1-demo` card-deck pattern with an `audio_file` field, keyed by `boehtlingk_num`.
...
This box (Windows, this worktree) has **no rclone install and no `~/.config/rclone/rclone.conf`** — [FINDINGS §719](https://github.com/gasyoun/Uprava/blob/main/FINDINGS.md)'s `yadisk:` remote is Mac-only. Fetched WebDAV credentials instead, over the already-trusted SSH key: `ssh root@193.232.229.92 "grep -E '^YANDEX_DISK_(LOGIN|APP_PASSWORD)=' /var/www/html/.env | base64 -w0"` (whole-blob base64, **not** line-wise — line-wise truncates the password per the same finding's gotcha), then plain `curl -X PROPFIND -u "$LOGIN:$PASS" -H "Depth: 1" https://webdav.yandex.ru/<folder>/` — no rclone needed, WebDAV is a raw HTTP verb. Scripts: [`../../tools_h4474_list_yadisk.py`](../../tools_h4474_list_yadisk.py), [`../../tools_h4474_list_sys.py`](../../tools_h4474_list_sys.py), [`../../tools_h4474_build_manifest.py`](../../tools_h4474_build_manifest.py), [`../../tools_h4474_add_duration.py`](../../tools_h4474_add_duration.py) (repo root of this worktree — move into a proper `scripts/` home if this line of work continues).
