...
This box (Windows, this worktree) has **no rclone install and no `~/.config/rclone/rclone.conf`** — [FINDINGS §719](https://github.com/gasyoun/Uprava/blob/main/FINDINGS.md)'s `yadisk:` remote is Mac-only. Fetched WebDAV credentials instead, over the already-trusted SSH key: `ssh root@193.232.229.92 "grep -E '^YANDEX_DISK_(LOGIN|APP_PASSWORD)=' /var/www/html/.env | base64 -w0"` (whole-blob base64, **not** line-wise — line-wise truncates the password per the same finding's gotcha), then plain `curl -X PROPFIND -u "$LOGIN:$PASS" -H "Depth: 1" https://webdav.yandex.ru/<folder>/` — no rclone needed, WebDAV is a raw HTTP verb. Scripts: [`scripts/tools_h4474_list_yadisk.py`](https://github.com/gasyoun/Systema-Sanscriticum/blob/h4474-subhashita-audio-srs-drain/docs/H4474-subhashita-audio-srs/scripts/tools_h4474_list_yadisk.py), [`scripts/tools_h4474_list_sys.py`](https://github.com/gasyoun/Systema-Sanscriticum/blob/h4474-subhashita-audio-srs-drain/docs/H4474-subhashita-audio-srs/scripts/tools_h4474_list_sys.py), [`scripts/tools_h4474_build_manifest.py`](https://github.com/gasyoun/Systema-Sanscriticum/blob/h4474-subhashita-audio-srs-drain/docs/H4474-subhashita-audio-srs/scripts/tools_h4474_build_manifest.py), [`scripts/tools_h4474_add_duration.py`](https://github.com/gasyoun/Systema-Sanscriticum/blob/h4474-subhashita-audio-srs-drain/docs/H4474-subhashita-audio-srs/scripts/tools_h4474_add_duration.py).
...
**If yes:** the next session executes [WIRING_PLAN.md](https://github.com/gasyoun/Systema-Sanscriticum/blob/h4474-subhashita-audio-srs-drain/docs/H4474-subhashita-audio-srs/WIRING_PLAN.md) directly — no further mapping work needed, the manifest above is ready to consume.
