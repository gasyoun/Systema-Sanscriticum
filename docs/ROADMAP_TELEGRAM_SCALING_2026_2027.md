...
> **Truth-pass 19-08-2026 (H3072, Opus 5 `claude-opus-5`):** зонтичный документ, и надстроенные им узкие планы ушли вперёд: WS1 (inbound-поддержка) живёт в [ROADMAP_SUPPORT_AUTOMATION_2026_2027.md](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/docs/ROADMAP_SUPPORT_AUTOMATION_2026_2027.md) и с 30-07 отгрузил двухканальные дневные сводки, шаблоны canreply, статус доставки ответа куратору и watchdog-cooldown синка. Зонтик не переписан сознательно — диагноз «архитектура сильная, активация низкая» всё ещё верен; за фактами идти в узкие планы.
...
## 8. Развилки — ✅ ВСЕ 4 РЕШЕНЫ MG 11-07-2026

Все четыре @DECIDE закрыты в интервью MG 11-07-2026 (Opus 4.8 `claude-opus-4-8`). Ниже —
рулинги; остаточные исполнительские действия — в §8a.

- **D1 — авто-ответ: категории и порог. ✅ РЕШЕНО.** Категории и порядок: **A Zoom → C расписание
  → B запись**. Порог промоушена категории из draft-only в auto: **≥ 95 % черновиков приняты
  БЕЗ правок** (`accepted / (accepted+edited+discarded)` по `SupportAiReplyEvent`), на выборке
  **≥ 50 черновиков и ≥ 2 недель**, при **нуле фактологических правок** в окне (куратор
  поправил ссылку/время хотя бы раз → счётчик сбрасывается). Промоушен **по одной категории**,
  рубильник сохраняется, **авто-демоушен** при всплеске переспросов/«ссылка не работает».
  Старт — с **A (Zoom)** (самая детерминированная, высокочастотная). Это изменение жёсткого
  принципа §4.2 — оформить формальным `/decision-record` (D##) до прод-включения.
- **D2 — единый раннер MTProto-синка. ✅ РЕШЕНО.** Канонический раннер — **cron/супервизор Ивана
  запускает Laravel-команду `php artisan telegram-support:sync`** (один код-путь; `TELEGRAM_SUPPORT_ENABLED`
  остаётся `true`). Отдельный самостоятельный MadelineProto-скрипт не держим. Харвестер
  (`telegram-harvest:sync`) — только за общим session-lock (`LocksMadelineSession`), никогда
  параллельно с саппорт-синком (иначе `AUTH_RESTART`). Это закрывает P0.3 «двух систем».
- **D3 — модель согласия для outbound. ✅ РЕШЕНО: явный opt-in.** Ключевое разграничение —
  **транзакционные ≠ маркетинговые**: подтверждения оплат/доступов, напоминания о долгах,
  алерты недобора группы = служебные, **opt-in не требуют** и продолжают идти. Явный opt-in
  нужен только для **промо/рассылок (WS2)**. Механика D3: (1) чекбокс согласия → хранимый
  `marketing_consent_at` (зеркалит открытый в GTD вопрос `pd_consent_at`); (2) строка
  «отписаться» в каждой рассылке + её соблюдение (`marketing_unsubscribed_at`); (3) недельный
  лимит частоты.
- **D4 — инструмент рассылок. ✅ РЕШЕНО: строить в Systema.** Сегментные рассылки — поверх
  существующего `SendMessengerAlerts` (уже мультиканальный TG+VK) + селектор сегмента
  (должники / холодные лиды / когорта) + гард согласия/opt-out из D3 + лимит частоты.
  Broadcast-скрипты Ивана НЕ оборачиваем — единый контур enforcement согласия (усиливает WS3).

## 8a. Остаточные действия после рулингов §8

- **D1 →** формальный `/decision-record` (D##) до прод-включения авто-ответа; инструментовать
  промоушен-гейт поверх S2-счётчиков (часть WS1.2).
- **D2 →** один вопрос Ивану (что именно крутит его cron; включён ли Laravel-планировщик
  `telegram-support:sync`) → затем агент проверяет [`Kernel.php`](https://github.com/gasyoun/Systema-Sanscriticum/blob/main/app/Console/Kernel.php)
  (сейчас `telegram-support:sync` в планировщике `everyMinute` при `TELEGRAM_SUPPORT_ENABLED=true`)
  и формализует харвестер-lock. GTD @DO.
- **D3/D4 →** handoff на сегментные рассылки (WS2.1/W2.2): миграция `marketing_consent_at`/
  `marketing_unsubscribed_at`, чекбокс согласия, opt-out, лимит частоты, селектор сегмента.
