...
return new class extends Migration
{
    ...
    {
        Schema::create('money_obligations', function (Blueprint $table) {
            $table->id();
            $table->string('obligation_key', 191)->unique();
        ...
        Schema::create('money_movements', function (Blueprint $table) {
            ...
            $table->string('movement_key', 191)->unique();
            ...
            $table->string('received_account', 32)->nullable();
            ...
            $table->string('evidence_key', 191)->nullable()->unique();
            $table->unsignedBigInteger('legacy_payment_id')->nullable()->index();
        ...
        Schema::create('money_allocations', function (Blueprint $table) {
            $table->id();
            ...
            $table->string('transfer_group', 191)->nullable()->index();
            $table->string('reason', 500)->nullable();
            $table->unsignedBigInteger('created_by')->nullable();
            $table->timestamp('created_at')->nullable();
        });
    ...
    private function triggers(): array
    {
        ...
        return [
            'money_movements_bi' => ['table' => 'money_movements', 'event' => 'INSERT', 'rules' => [
                ...
                ['ledger: correction must precede the reversal of the corrected row', "NEW.type = 'correction' AND EXISTS (SELECT 1 FROM money_movements r WHERE r.reverses_movement_id = NEW.corrects_movement_id)"],
            ...
            'money_allocations_bi' => ['table' => 'money_allocations', 'event' => 'INSERT', 'rules' => [
                ...
                ['ledger: allocation and obligation belong to different students', 'EXISTS (SELECT 1 FROM money_movements m, money_obligations o WHERE m.id = NEW.movement_id AND o.id = NEW.obligation_id AND m.user_id IS NOT NULL AND m.user_id <> o.user_id)'],
