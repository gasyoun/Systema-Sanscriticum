 ...
 *
 * H3802 (прод 31-08-2026): именно потому, что H3371 сам называет лаг «до
 * часов», а не минут, длинные паузы окна почти никогда не успевают —
 * задержки после первой пробы стабильно сгорали впустую (≈2/3 бюджета
 * TimeoutStartSec каждого часового прогона), группа из 115 частей грузилась
 * ~2 части/час вместо возможных ~4+ и юнит гарантированно ловил systemd
 * kill каждый час. Настоящая повторная проверка живёт не здесь, а в
 * resumePendingGroups(): она листит off-site ЗАНОВО на следующем прогоне,
 * так что часть, ставшая видимой позже, никогда не перезаливается вторым
 * разом — окно проб сокращено до одной мгновенной пробы (ловит честные
 * пропажи 2xx-без-сохранения), а докатка лага консистентности отдана
 * следующему часовому листингу, где она и так уже происходила бесплатно.
 *
...
class SplitUploadToYandex
{
     ...
     * прогона (H3802: ≈2/3 времени на доказанно-бесполезное ожидание,
     * из-за чего 115-частная группа грузилась ~2 части/час вместо ~4+).
     * Одна мгновенная проба ловит честные пропажи (Яндекс 2xx без записи);
     * докатку лага консистентности берёт на себя resumePendingGroups() на
     * СЛЕДУЮЩЕМ часовом прогоне — она и так листит off-site заново.
     */
    private const VERIFY_PROBE_DELAYS = [0];
