 ...
 * Lead::markConverted() отсюда НЕ вызывается — это делает Payment: всегда на
 * депозит/пробное/марафон, и на обычную покупку курса только за флагом
 * features.lead_converted_at_on_course_paid (default OFF, H2186 / §2.4 residual).
 * Мост на converted_at не опирается и его не чинит.
 *
...
class PaymentDealBridgeObserver
{
     ...
     *
     * Но НЕ по одной только группе: курс проверяется и здесь — см. dealOfPlan.
     */
    private function closeOrRecordDeal(Payment $payment): void
    {
        ...
        if ($group !== null) {
            ...
            $planOwnsADeal = Deal::query()->where('installment_group_id', $group)->exists();
        }

        // ГАРД КУРСА ОТВЕРГ СДЕЛКУ ПЛАНА — ЗНАЧИТ ЧУЖУЮ БРАТЬ ТЕМ БОЛЕЕ НЕЛЬЗЯ
        // (найдено вторым adversarial-ревью, уже на самом H1690).
        //
        // dealOfPlan просматривает ВСЮ группу, не только открытые сделки. Раз
        // он вернул null при непустой группе, ни одна сделка плана по курсу не
        // подошла — а OR-условие findOpenDealFor («группа пуста ИЛИ равна G»)
        // после этого способно найти ТОЛЬКО сделку БЕЗ группы. То есть каждый
        // раз, когда срабатывает новый гард, цель провала — заведомо чужая
        // сделка, и closeDealWith проставил бы ей штамп плана G навсегда.
        // Ровно то «закрытие чужой строки», которое запрещает докблок
        // findOpenDealFor, только пришедшее с другой стороны.
        //
        // Поэтому: план уже владеет сделкой, а этот платёж по курсу с ней
        // разошёлся → он сам себе продажа, заводим новую сделку с тем же
        // штампом группы. Это и есть обещанная «вторая, ВИДИМАЯ сделка на ту
        // же группу»: следующий взнос того же курса найдёт её через dealOfPlan
        // напрямую. Первый взнос плана (группы в deals ещё нет) по-прежнему
        // штатно закрывает менеджерскую открытую сделку — эта ветка его не
        // касается.
        $deal = $planOwnsADeal ? null : $this->findOpenDealFor($payment, $group);
    ...
    /**
     * Сделка этого плана рассрочки. КУРС ПРОВЕРЯЕТСЯ И ЗДЕСЬ (ruling H1690 по
     * находке adversarial-ревью H1659, оценённой как PLAUSIBLE).
     *
     * Ревью показало: до H1690 ветка группы искала сделку ОДНИМ условием
     * where('installment_group_id', $group) — без курса, человека и лида. Сделку,
     * помеченную планом G на курс A, реверснули, а потом руками перевели на курс
     * B — и второй взнос плана G (деньги за курс A) её закрывал.
     *
     * Развилка была из двух прочтений, и выбрано второе:
     *  (а) «тождество плана сильнее»: сделка принадлежит плану, курс ей поменял
     *      человек, значит он сам и порвал связь;
     *  (б) КУРС — РАЗЛИЧАЮЩИЙ ПРИЗНАК ВЕЗДЕ, без исключений для этой ветки.
     *
     * Почему (б). Цена ошибки несимметрична. По (а) выигранная сделка пишется на
     * курс B, за который в этой транзакции никто не платил, и одновременно
     * НАСТОЯЩАЯ продажа курса A исчезает из воронки — две порчи отчётности разом
     * и обе молчаливые. По (б) худший исход — вторая сделка на ту же группу,
     * зато деньги легли на свой курс: ошибка видимая и исправимая. Это верно
     * ТОЛЬКО вместе с гардом $planOwnsADeal в closeOrRecordDeal: без него отказ
     * по курсу проваливался в findOpenDealFor и угонял чужую сделку вместо того,
     * чтобы завести свою (adversarial-ревью H1690). Ровно тот же
     * приоритет уже зафиксирован в докблоке findOpenDealFor («закрывать оплатой
     * курса B сделку по курсу A — запись не в ту строку», дефект 1 ревью H1641);
     * до H1690 ветка группы была из этого инварианта тихо исключена.
     *
     * Терпимость к null — зеркало findOpenDealFor, чтобы (б) не начала плодить
     * дубли на ровном месте: сделка этого плана с ЕЩЁ НЕ проставленным курсом
     * может быть «той самой», а платёж без курса своей группе доверяет целиком.
     *
     * Порядок по id, а не oldest()/created_at: created_at секундной точности и
     * на MySQL, и на SQLite, так что два взноса одного вебхука в одну секунду
     * давали неустойчивый тай-брейк; id монотонен (пункт 3 H1690).
     *
     * Только чтение.
     */
    private function dealOfPlan(Payment $payment, string $group): ?Deal
    {
        $base = Deal::query()->where('installment_group_id', $group);

        if (! $payment->course_id) {
            return $base->orderBy('id')->first();
        }

        $exact = (clone $base)->where('course_id', $payment->course_id)->orderBy('id')->first();
        if ($exact !== null) {
            return $exact;
        }

        return (clone $base)->whereNull('course_id')->orderBy('id')->first();
    }
    ...
    private function installmentGroupFor(Payment $payment): ?string
    {
        ...
        //
        // ЗАЧЕМ ОН НУЖЕН, если первый переход почти всегда срабатывает раньше
        // (вопрос ревью H1659, разобран в H1690 и оставлен ОСОЗНАННО). Правда
        // в том, что PromiseAutoFulfiller::handlePaidPayment выходит на
        // linked_promise_id === null, — значит там, где fulfilled_payment_id
        // ставит ОН, первый переход уже дал ту же группу, и второй холостой.
        // Живой случай ровно один и он не гипотетический: платёж, созданный
        // кураторским PromiseFulfillment::fulfil (он связывает обещание ПОСЛЕ
        // создания платежа), откатили и провели заново. На повторном paid
        // прямой связи по-прежнему нет, а третий переход уже молчит — обещание
        // к тому моменту FULFILLED, не active/expired. То есть для «кураторский
        // взнос → реверс → повторная оплата» этот переход — единственный.
        // Стоимость — один SELECT по индексу; тест ниже ходит именно этим
        // сценарием, а не синтетической связью через withoutEvents. Это
        // ИЗМЕРЕНО, а не выведено: с вырезанным переходом тот тест даёт 2
        // сделки вместо 1 (мутационная проверка H1690).
        $group = PaymentPromise::query()
