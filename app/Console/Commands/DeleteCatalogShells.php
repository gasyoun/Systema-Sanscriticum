 ...
 *
 * H3122 намеренно отгрузил только отчет: «сперва аудит», поверх условия MG
 ...
 *
 * 1. **Вердикт не передается из отчета, а пересчитывается здесь.** Команда
 *    заново гоняет {@see CatalogShellAudit} в момент удаления. Отчет, снятый
 *    вчера (или пять минут назад), к этому моменту мог устареть: на курс успели
 *    записаться, завести тариф, выдать сертификат. Доверять сохраненной строке
 *    отчета — ровно тот класс ошибки, ради которого аудит и писался.
 * 2. **Идентификаторы обязательны и перечисляются руками.** Нет режима «удали
 *    все, что безопасно»: список объектов называет человек, а команда лишь
...
class DeleteCatalogShells extends Command
{
    protected $signature = 'catalog:delete-shells
        ...
        {--alias-into=* : Кому достается слаг удаляемого курса, «оболочка:живой курс» (напр. 421:335)}
    ...
    public function handle(CatalogShellAudit $audit, CatalogShellRetirement $retirement): int
    {
        ...
        if ($courseIds === [] && $groupIds === []) {
            $this->error('Не назван ни один объект. Укажите --course= и/или --group= явно: режима «удали все безопасное» здесь нет by design.');

            return self::FAILURE;
        }

        // Вердикт СЕЙЧАС, а не из отчета.
        $report = $audit->report();
        ...
        foreach ($groupIds as $id) {
            if (! in_array($id, $safeGroups, true)) {
                $refused[] = "группа {$id} — аудит НЕ считает ее безопасной пустой группой прямо сейчас";
            }
        }

        if ($refused !== []) {
            $this->error('ОТКАЗ — ни один объект не удален:');
        ...
        // Кому достается слаг. Считается ДО плана: без адресата удаление не идет.
        ...
        $removed = DB::transaction(function () use ($courseIds, $groupIds, $slugHomes, $retirement, &$adopted): array {
            ...
            foreach ($courseIds as $id) {
                // Слаг переселяется ДО снятия связок: иначе purge сотрет и его,
    ...
    /**
     * Кому достается слаг каждого удаляемого курса.
     *
     ...
     *
     * Порядок: явное `--alias-into=421:335` побеждает; иначе берется
     * ЕДИНСТВЕННЫЙ живой курс семьи; если их ноль или несколько — отказ, пока
     * человек не назовет адресата сам либо не отпустит слаг через
     * `--drop-slug=421`.
     *
    ...
    private function slugHomes(array $courseIds, CatalogShellRetirement $retirement): ?array
    {
        ...
        foreach ($courseIds as $id) {
            ...
            // Несколько кандидатов — молча выбирать нельзя: 301 увел бы людей на
        ...
        if ($problems !== []) {
            $this->error('ОТКАЗ — ни один объект не удален (слагу некуда переехать, ссылки стали бы 404):');
    ...
    /**
     * Снять связки объекта. Возвращает число удаленных строк.
     *
