 ...
 *    пишет человек, демон только раздает — и потому имеет право писать. Файл
 ...
 *  - КАМПАНИЯ: `--user=` / `--users-file=` + `--reason=` — выдает конкретному
 *    списку с собственной меткой. Это условие приемки хендоффа: D6 = вариант
...
class GrantFreeTierLesson extends Command
{
    ...
    public function handle(FreeTierLessonGranter $granter): int
    {
        ...
        if (! $granter->enabled()) {
            $this->warn('Флаг features.membership_free_tier ВЫКЛЮЧЕН — только отчет, ничего не пишем.');
            $apply = false;
        }
        ...
        if (! $campaign) {
            ...
            if ($cohortPath !== null) {
                ...
                // опечаткой, снесенный файл, файл без прав чтения у www-data —
                // все это НЕ ДОЛЖНО откатываться в авто-подбор, иначе одна
                ...
                if ($users === null) {
                    ...
                    $this->line('  Файл читает www-data (планировщик), поэтому он живет в storage/app, а не в /root.');
                ...
                if ($users->count() > $limit) {
                    // Явно, а не молча: усеченная когорта читается как «выдали
                    // всем», если про обрезку не сказать.
                    $this->warn('Когорта '.$users->count().' человек, за прогон берем '.$limit
                ...
                // платил» и берет верхние N. На проде это 923 человека против 372
                ...
                // кампании. Отчет остается всегда, чтобы демон не стал тишиной.
                if ($apply && ! (bool) config('membership.free_tier.daemon_apply', false)) {
                    $this->warn('РЕЖИМ ДЕМОНА: авто-выдача запрещена (membership.free_tier.daemon_apply = false) — только отчет.');
     ...
     *
     * NULL здесь означает «не смог», а НЕ «никого не нашел»: вызывающий обязан
     ...
     * список давал бы разный охват в зависимости от того, каким путем он попал
     * в команду.
     *
