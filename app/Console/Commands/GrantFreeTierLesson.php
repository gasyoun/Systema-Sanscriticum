 ...
 *    кандидатов сам, начиная с самых уснувших. **С H2899 он по умолчанию
 *    только ОТЧИТЫВАЕТСЯ**: его правило отбора не знает про давность, поэтому
 *    на проде оно видит 923 человека против 372 в той двенадцатимесячной
 *    когорте, которую документы называют «350». Авто-выдача включается
 *    отдельно — `FREE_TIER_DAEMON_APPLY=true`;
...
class GrantFreeTierLesson extends Command
{
    ...
    public function handle(FreeTierLessonGranter $granter): int
    {
        ...
        if (! $campaign) {
            // H2899. Демон САМ выбирает кандидатов, и его правило не знает про
            // давность: `eligibleUserIds()` сортирует по «давно не платил» и
            // берёт верхние N. На проде это 923 человека против 372 в той
            // двенадцатимесячной когорте, которую документы называют «350».
            // Раздача в 2,5 раза шире написанного, по 200 в сутки и молча, —
            // не то, что должен делать флаг, названный именем кампании.
            // Поэтому авто-выдача требует ОТДЕЛЬНОГО разрешения; отчёт остаётся
            // всегда, чтобы демон не превратился в тишину.
            if ($apply && ! (bool) config('membership.free_tier.daemon_apply', false)) {
                ...
                $this->line('  Бесплатный уровень доставляется КАМПАНИЕЙ с явным списком:');
                $this->line('    membership:grant-free-lesson --users-file=<файл> --reason=<метка> --apply');
                $this->line('  Включить авто-выдачу: FREE_TIER_DAEMON_APPLY=true (кому дарить — решение человека, не сортировки).');
                $apply = false;
            }
