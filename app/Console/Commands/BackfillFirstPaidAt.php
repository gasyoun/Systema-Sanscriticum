 ...
 *       отмененных.
 *   (b) платеж СЕЙЧАС оплачен, но аудит-следа с paid-эвиденцией нет (до-
 *       аудитная эпоха, ни разу не отменялся) — берем created_at платежа как
 *       единственную достоверную нижнюю границу.
 *
 * Остаток, который НЕЛЬЗЯ восстановить: платеж сейчас НЕ оплачен и аудит не
 * содержит paid-эвиденции — значит, он был оплачен и отменен целиком до
 * 08-06-2026, не оставив следа. Остается null; количество (не построчно)
...
class BackfillFirstPaidAt extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->info(sprintf(
            'Остаток без следа (отменен до 08-06-2026, first_paid_at останется null): %d',
            $residualNoTrace
        ));
