...
class RemindDebtors extends Command
{
     ...
     * «Должники»). Реф-блок уже идет/просрочен — напоминаем; если реф-блок еще
     ...
     * студент еще не оплатил. Покрывает кейс «текущий блок оплачен, следующий
     ...
     * дедлайну блока; частоту по-прежнему задает cadence — эскалация происходит
    ...
    private function deliver(User $user, int $courseId, ?int $block): void
    {
        ...
        // DunningStage) починкой отчета H2746 было бы подменой политики.
     ...
     * не отличит от «никогда не платил» без этого расчета). Готовое
     ...
     * «до дня старта следующего модуля, до 00:00 по Москве»). Берем starts_at
