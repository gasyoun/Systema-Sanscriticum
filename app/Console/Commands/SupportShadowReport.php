 ...
 * ни то ни другое не говорит, кто был прав. Отчет существует, чтобы человек
 * увидел числа перед живым включением автоотправки (R9); решение остается за
...
class SupportShadowReport extends Command
{
    protected $signature = 'support:shadow-report
        ...
        {--write= : Записать отчет в файл (по умолчанию docs/RESULTS_SUPPORT_SHADOW_WEEK_<дата>.md)}
    ...
    public function handle(): int
    {
        ...
        if ($shadow->isEmpty()) {
            ...
            $lines[] = '';
            $lines[] = '**Живое включение автоотправки на пустом отчете недопустимо** (рулинг R9): решать не по чему.';
            $lines[] = '';
    ...
    private function silenceSection(array $rows): array
    {
        ...
        return [
            '## 3. Тишина — та самая работа, ради которой все затевалось',
            '',
            sprintf(
