 ...
 *
 * Пока связки нет, «Взаимозачет» и сверка выплат преподавателя его вообще не
 * видят: платеж-«Расход», заведенный на личного пользователя преподавателя,
 ...
 *     в отчет словами «неоднозначно», а не тихим выбором первого;
 ...
 * `payment_expense_direct`. Их не надо подтверждать в очереди атрибуции еще
...
class LinkTeacherUsers extends Command
{
    ...
    public function handle(): int
    {
        ...
        if ($toWrite !== []) {
            $this->line('');
            $this->warn(
                'Внимание: после связки платежи-«Расходы», заведенные на этих пользователей, '
    ...
    /**
     * ФИО к сравнимому виду: регистр, е→е, схлопнутые пробелы, снятая
