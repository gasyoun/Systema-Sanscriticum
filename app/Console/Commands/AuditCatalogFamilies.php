 ...
 * чтобы отчет в репозитории был воспроизводим одной командой, а не собран
 * руками.
 */
class AuditCatalogFamilies extends Command
{
    protected $signature = 'catalog:audit-families
        ...
        {--markdown : Готовый текст отчета для docs/}
        {--all : В markdown печатать и семьи из одного курса}';

    protected $description = 'Вердикт по каждой семье курсов: streams | duplicate | unique. Только отчет, ничего не меняет.';
    ...
    /**
     * Метка вердикта. У `duplicate` она несет ЭКСПОЗИЦИЮ, потому что без нее
    ...
    /**
     * Текст отчета. Семьи из одного курса по умолчанию сворачиваются в один
    ...
    private function markdown(array $rows, bool $all): string
    {
        ...
        $out[] = '';
        $out[] = 'Отчет команды `php artisan catalog:audit-families --markdown`. Только чтение: ни одной записи в `courses`/`tariffs` не сделано.';
        $out[] = '';
        ...
        if ($singles !== []) {
            ...
            $out[] = '';
            $out[] = 'Разбора не требуют; перечислены, чтобы охват отчета был полным — каждый курс каталога попал ровно в одну строку.';
            $out[] = '';
        ...
        $out[] = '';
        $out[] = '- **streams** — несколько строк `courses`, и каждая отличима как самостоятельный поток: у нее есть собственные данные (роль `live`/`recording`) и собственный ключ потока — номер из названия либо дата первого платежа. Законно; складывать потоки между собой в отчетности нельзя (семантика `App\Support\CourseCadence`).';
        $out[] = '- **duplicate** — хотя бы одна строка не отличима: либо у нее нет ни блоков, ни активных тарифов, ни оплат, либо две строки претендуют на один и тот же поток.';
        ...
        $out[] = '';
        $out[] = 'Разделение добавлено 31-08-2026 по боевому промаху: первая версия отчета звала «свести витрину и SEO» для всех четырех `duplicate`, а три из них были скрыты и уже отдавали 404 — совет посылал человека чинить то, чего не существует.';
        $out[] = '';
     ...
     * следующем прогоне вывод разошелся бы с цифрами над ним.
     *
     * @param  list<array<string, mixed>>  $rows
     * @return list<string>
     */
    private function classBreakdown(array $rows): array
    {
        ...
        $labels = [
            ...
            CatalogFamilyAudit::CLASS_CURATOR_GATED_SALE => '**НЕ ДЕФЕКТ И НЕ КАНДИДАТ НА УБОРКУ: курируемая продажа по прямой ссылке.** Курс скрыт с витрины, но `is_active` и с активными тарифами — значит он ПРОДАЕТСЯ: `/checkout/{tariff}` связывает тариф и никогда не читает `Course.is_visible`. Так куратор продает запись доверенному студенту на ограниченный срок. `tariffs.is_active` — гейт ПОКУПКИ, а не доступа; гасить тарифы, скрывать еще сильнее или сводить строку через `catalog:retire-shell` значит закрыть работающую продажу (курс 327, 31-08-2026)',
        ];
    ...
    private function evidenceCell(array $members, array $reasons): string
    {
        ...
        foreach ($members as $member) {
            $parts[] = sprintf(
                '%d: %s · блоков %d · активных тарифов %d (%s) · оплат %d · записано %d · первый платеж %s · группы: %s',
                $member['id'],
                $member['url'],
                $member['blocks'],
