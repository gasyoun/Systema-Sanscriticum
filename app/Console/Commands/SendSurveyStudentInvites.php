 ...
 *
 * Кого берем (текущие ученики, а не «любые платившие когда-то»):
 ...
 *   ИЛИ оплата за последние 6 месяцев (Payment::PAID_STATUSES) — свежий платеж
 ...
 *   - приглашенные в кабинет за 3 месяца (users.cabinet_invite_sent_at);
 *   - приглашенные в саппорт-боте за 3 месяца (link_invited_at) — консервативная
 ...
 *   - уже приглашенные в ЛЮБУЮ волну опроса (survey_invitations: queued/sent/
 *     unknown; failed — разрешен повтор, отправка точно не случилась).
 *
...
class SendSurveyStudentInvites extends Command
{
     ...
     * поверх предыдущего, счетчик этапа — дельта с предыдущей ступенью.
     *
     ...
     * unknown никогда не перезаписываются: updateOrCreate здесь запрещен —
    ...
    private function verifyBotIdentity(): bool
    {
        ...
        $this->info("Бот подтвержден: @{$username} (id ".$response->json('result.id').').');
    ...
    private function invitationText(string $slug): string
    {
        ...
        return 'Вы уже учитесь у нас — помогите понять, что действительно повлияло на ваш выбор '
            .'и что стоит улучшить. Подробная анкета о вашем пути, покупках и результатах займет около '
