...
final class MoneyLedgerBackfillReport extends Command
{
    ...
    public function handle(LegacyLedgerMapper $mapper, LedgerService $ledger, LedgerProjection $projection): int
    {
        ...
        $run = function () use ($mapper, $ledger, $projection, $shadow): array {
            return $this->walk($mapper, $shadow ? $ledger : null, $projection);
        };

        $report += $shadow ? $ledger->shadow(fn () => $run()) : $run();
    ...
    private function walk(LegacyLedgerMapper $mapper, ?LedgerService $ledger, LedgerProjection $projection): array
    {
        ...
        $shadowStats = ['posted' => 0, 'rejected' => 0, 'ledger_net_kopecks' => 0];
        ...
        foreach ($mapper->families() as $plan) {
            ...
            if ($ledger !== null) {
                try {
                    $net = DB::transaction(fn () => $this->post($ledger, $projection, $plan));
                    $shadowStats['posted']++;
        ...
        if ($ledger !== null) {
            $shadowStats['integrity_breaches'] = $projection->integrityBreaches();
            $shadowStats['control_totals'] = $projection->controlTotals();
    ...
    /**
     * Проводит один план через сервис (внутри теневой транзакции).
     *
    ...
    private function render(array $r, int $limit): void
    {
        ...
        if (isset($r['shadow'])) {
            ...
            $this->line("Теневой прогон: проведено {$s['posted']}, отклонено триггерами {$s['rejected']}, нетто ядра ".$this->rub($s['ledger_net_kopecks'])
                .' · нарушения целостности: '.json_encode(array_map('count', $s['integrity_breaches'])));
        }
    }
