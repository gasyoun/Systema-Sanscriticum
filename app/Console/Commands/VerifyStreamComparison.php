 ...
 *
 * Эталон живет здесь, а не в тесте, потому что проверяется он на боевой базе,
...
class VerifyStreamComparison extends Command
{
    ...
    protected $description = 'Сверить отчет «Потоки курса» с эталоном, снятым с прода';

    /**
     * Эталон: слаг семьи => ожидаемые значения.
     *
     * `buyers` — сколько человек купили ИМЕННО этот блок (то же определение,
     * что у выручки блока). Счет «имеют доступ» отличается на купивших курс
     ...
     * схемы ЗП дает 0 — в волне 1 это правильный результат, а не баг (фенс).
     *
    ...
    private function compare(string $family, array $expected, array $actual): int
    {
        ...
        $this->line(sprintf(
            'Начислено (валовое) %s ₽ · выплачено подтвержденно %s ₽ · остаток %s ₽ (%s) · ждут подтверждения %s ₽ в %d строках.',
