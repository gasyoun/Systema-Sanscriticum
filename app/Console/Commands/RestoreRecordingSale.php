 ...
 * который купили **129 раз**. Формально все верно: курс был скрыт с витрины,
 ...
 * проставлен `recording_of_course_id`. Это ровно то, о чем был рулинг — запись
 * внутри карточки живого потока, — и это не дает команде стать универсальным
...
class RestoreRecordingSale extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->newLine();
        $this->info(sprintf(
            'Готово: тарифов включено %d. Второй карточки в каталоге не появится — курс остается записью курса %d, canonical на него же.',
            count($requested),
            $live?->id,
        ));
