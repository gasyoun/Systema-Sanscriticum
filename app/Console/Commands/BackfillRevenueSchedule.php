 ...
 * Обсервер строит строки только на НОВЫЕ/измененные платежи — эта команда
 * покрывает все, что было оплачено ДО внедрения accrual. Идемпотентна:
 * пересобирает субледжер с нуля (regenerateFor = delete+insert на платеж),
 * поэтому ее можно гонять повторно для лечения дрейфа.
 *
