 ...
 * есть. Тот же класс тихой лжи, что H3812 в деньгах: восемь зелёных тестов
 * проверяли половину контракта.
 *
 * Поэтому здесь ДВА независимых счёта, и они обязаны сойтись:
 ...
 *    поэтому второе тело затирает первое и в индексе остаётся одно.
 *
 ...
 * дублируются). В retrieval он не идёт намеренно — это не ответ студенту, а
 * инструкция для того, кто правит корпус, и в промпте студентской модели ей
 * не место. Но и молчать о ней нельзя: команда печатает её объём отдельной
...
class KnowledgeCoverage extends Command
{
    ...
    public function handle(FaqCorpusParser $parser): int
    {
        ...
        // Проверка эмбеддингов идёт по ТОМУ ЖЕ набору файлов, что и скан
        // потерь: с --path отчёт «проэмбедено» обязан говорить про этот файл, а
    ...
    private function scanFile(FaqCorpusParser $parser, string $file): array
    {
        ...
        // заголовка» — отчёт показывал бы 134 символа шапки там, где шапки нет
        ...
        foreach ($lines as $line) {
            ...
            if (preg_match('/^#\s+/u', $line)) {
                ...
                // молча пропадал бы из обоих счётов сразу.
                continue;
            }
            if (! $seenHeading) {
                $preface[] = $line;
