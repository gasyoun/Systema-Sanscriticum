...
/**
 * H3765 A1: перепись задержки приема входящих поддержки — ЧИТАЮЩАЯ, ничего
 * не меняет и никуда не шлет.
 *
 * Зачем. Автоответчик пропускает как «протухшее» все, что старше
 ...
 *  - ЛАГ ПРИЕМА (sent_at → created_at) — сколько сообщение пролежало у
 ...
 *    сами тянули после приема. Большой лаг = проблема такта/очереди.
 * Первое — не наша вина, второе — наша.
 *
 * Считаем в PHP, а не в SQL: TIMESTAMPDIFF есть в MySQL и нет в SQLite, на
 * котором идут тесты, а объем окна (тысячи строк) в память укладывается.
 */
class SupportIngestLatencyReport extends Command
{
    protected $signature = 'support:ingest-latency-report
        ...
        {--write= : Записать отчет в файл (по умолчанию docs/RESULTS_SUPPORT_INGEST_LATENCY_<дата>.md)}
        {--no-write : Только показать в консоли}';

    protected $description = 'H3765 A1: перепись лага приема и лага обработки входящих Telegram-поддержки; называет причину stale-пропусков. Только чтение.';

    /** Лаг приема ниже этого — сообщение считается живым, а не дозабором истории. */
    ...
    public function handle(): int
    {
        ...
        $lines = [];
        $lines[] = '# RESULTS — перепись лага приема Telegram-поддержки (H3765 A1)';
        $lines[] = '';
    ...
    private function dailySection($incoming): array
    {
        ...
        $lines = ['## 1. По дням приема: живое против дозабора истории', ''];
        $lines[] = 'Живым считаем сообщение с лагом приема меньше часа: оно попало к нам примерно тогда,';
        $lines[] = 'когда студент его отправил. Все остальное — история, приехавшая задним числом.';
        $lines[] = '';
        $lines[] = '| День приема | Живых | Дозабор | Медиана лага приема | Самое старое `sent_at` в порции |';
        $lines[] = '|---|---:|---:|---:|---|';
    ...
    private function accountSection($incoming, array $accounts): array
    {
        ...
        $lines[] = '| Аккаунт | Входящих | Живых | Медиана лага приема |';
        $lines[] = '|---|---:|---:|---:|';
    ...
    private function staleBandSection($events, $messageById): array
    {
        ...
        $lines[] = 'что вернул бы широкий потолок подсказок (A2); все правее — история.';
        $lines[] = '';
    ...
    private function verdictSection($events, $messageById, array $processing, $incoming): array
    {
        ...
        if ($medianProcessing > self::CADENCE_BUG_PROCESSING_LAG_MINUTES) {
            $cause = 'ТАКТ СИНКА. Медиана лага обработки — '.$this->humanMinutes($medianProcessing)
                .', то есть сообщение лежит в базе необработанным дольше часа. Это наша задержка, ее и надо чинить;'
                .' потолок свежести здесь ни при чем.';
            ...
            $cause = 'ПОЗДНЯЯ ДОСТАВКА. Обработка успевает, но пропущенное сосредоточено в свежих полосах —'
                .' смотрите на перебои приема (перезапуски сессии, watchdog-убийства).';
        }
        ...
        $lines[] = '';
        $lines[] = sprintf(
            'Что вернет расширение потолка подсказок до 24 ч: **%d** сообщений за окно из %d пропущенных (%s) —'
