 ...
 *
 * Нужна там, где курс уже удален, а ссылки на него остались. Ровно этот случай
 * случился 31-08-2026: `catalog:delete-shells` снес курсы 421 и 430 вместе с их
 ...
 * (слаг переселяется до удаления), но уже мертвые ссылки чинить нечем — этим.
 *
 * Чужое не перевешивает: слаг, который уже канон другого курса или алиас
 * третьего, остается как есть, и команда говорит об этом вслух.
 */
class AliasCourseSlug extends Command
{
    ...
    public function handle(CatalogShellRetirement $retirement): int
    {
        ...
        if ($resolved !== null) {
            $this->warn(sprintf(
                'Слаг /k/%s уже ведет на курс %d «%s» — не трогаем.',
