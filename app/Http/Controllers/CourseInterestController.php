...
class CourseInterestController extends Controller
{
     ...
     */
    private function studentKocherginaGroupName(?\App\Models\User $user, ?Course $targetCourse): ?string
    {
        if ($user === null || $targetCourse === null) {
            return null;
        }

        $group = \App\Models\Group::query()
        ...
        $course = $group->courses->firstWhere(
            fn (Course $c): bool => $c->id !== $targetCourse->id
                && \App\Services\Schedule\TextbookScale::courseFamilyPublic((string) $c->title) === 'kochergina',
        );
