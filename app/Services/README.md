...
- Все операции обёрнуты в `try/catch` — сбой трекинга не ломает основной флоу.
...
- Распознаёт все варианты URL: `/watch?v=`, `/embed/`, `youtu.be/`, голые ID.
...
| `LecturePublisher` | Перемещает HTML и слайды из `storage/` в `public/lectures/`, переписывает пути к ассетам, создаёт `Lesson`, переводит черновик в статус `published`. |
...
| Класс | Что делает |
|---|---|
| `ScheduleGenerator` | Создаёт события календаря из конфигурации. Уважает фильтры дней недели, пропуски, дополнительные даты, плейсхолдеры шаблона (`{N}`, `{DATE}`, `{BLOCK}`, `{BN}`). |
