...
class TelegramAdminNotifier
{
    ...
    /**
     * Оборвалась ли сеть на последнем send(): ответа от Telegram не было вовсе.
     * Нужен вызывающим с TelegramSendGuard — при отказе Telegram клейм
     * отпускают, а при обрыве держат (сообщение могло уйти).
     */
    public function lastSendWasNetworkFailure(): bool
    {
        return $this->lastSendWasNetworkFailure;
    }
