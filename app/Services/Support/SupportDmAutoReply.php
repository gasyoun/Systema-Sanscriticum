...
final class SupportDmAutoReply
{
    ...
    {
        $chatIds = config('services.telegram_support.hint_suppressed_chat_ids', []);
        if (is_array($chatIds) && in_array((int) $incoming->telegram_chat_id, array_map('intval', $chatIds), true)) {
            return 'service_chat';
        }

        $prefixes = config('services.telegram_support.hint_suppressed_prefixes', []);
    ...
    {
        return (bool) config('services.telegram_support.hint_dedup_enabled', true);
    }
