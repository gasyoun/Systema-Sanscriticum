...
class SupportAnswerFactResolver
{
    ...
    private function resolveScheduleChange(User $user): ?array
    {
        ...
        foreach ($changes as $change) {
            ...
            $lines[] = match ($change['kind']) {
                ...
                'moved' => "· занятие{$title} изменилось — сейчас в расписании {$when} (МСК)",
    ...
    private function scheduleChangeKind(Schedule $class, \DateTimeInterface $since): ?string
    {
        ...
        // Прежнее время занятия НИГДЕ не хранится, поэтому отличить перенос от
        // правки ссылки таблица не позволяет — и черновик не утверждает, что
        // занятие «перенесено», а говорит «изменилось — сейчас в расписании
        // такое-то время». Это верно в обоих случаях.
