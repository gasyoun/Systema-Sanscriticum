...
final class LedgerService
{
    ...
    /** Перенос снятых сторно распределений на корректировку притока (идемпотентен по ключам). */
    private function carryAllocations(MoneyMovement $reversal, MoneyMovement $correction, string $key, ?int $by): void
    {
        ...
        // Не больше новой суммы и не больше свободного остатка семьи (возвраты уже вычтены).
        $left = min($correction->amount_kopecks, max(0, $this->projection->familyResidue((int) $correction->cap_anchor_id, forUpdate: true)));
        foreach ($held as $row) {
