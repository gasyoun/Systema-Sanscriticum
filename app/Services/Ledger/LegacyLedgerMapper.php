...
final class LegacyLedgerMapper
{
    ...
    public function plan(Payment $p, array $refunds = []): array
    {
        ...
        if ($discount === 0 && self::positiveDecimal($p->getRawOriginal('discount_percent'))) {
        ...
        if ($p->tariff === 'deposit') {
            ...
            if (self::positiveDecimal($p->getRawOriginal('consumed_amount'))) {
    ...
    /** «> 0» для десятичной строки из БД без float: не отрицательная и есть ненулевая цифра. */
    private static function positiveDecimal(mixed $raw): bool
    {
        $s = trim((string) $raw);

        return $s !== '' && ! str_starts_with($s, '-') && preg_match('/[1-9]/', $s) === 1;
    }
