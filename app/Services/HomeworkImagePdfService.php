...
class HomeworkImagePdfService
{
     ...
     *
     * Гард стоит ДО декодирования: габариты читаются из заголовка, и если
     * оценка декода не влезает в остаток memory_limit, кадр вообще не
     * открывается — заглушка вместо фатального OOM всего POST.
     */
    public function convertToPdfJpeg(string $bytes, string $mime): ?string
    {
        ...
        // Никакого «отдать как есть» для крупного кадра здесь нет: именно этот
        // путь и раздувал сборку до OOM.
