...
| 73 | **H2444 упражнения из файлов занятия хинди** | ✅ **включено 14-08-2026.** `HINDI_ATTACHMENT_DRILLS=true` + `config:cache`. `config:show features.hindi_attachment_drills` → true. Census: 130 lessons / 3 with files / 1 extractable (sidecar H2731) / 1 with items. Probe lesson 1723: 12 attachment items, PDF extractable. In-process GET `/c/hindi-5-vt1500-2026/u/1723/drills` as paid user 6767 → 200 (handout + items); guest → 302. Откат: флаг `false` + `config:cache`. | ⚙️ флаг (без денег) |
