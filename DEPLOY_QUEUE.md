...
_Создано: 08-07-2026 · Обновлено: 14-08-2026 (№76 H2731 `hindi:pdf-sidecar` one-file apply, flag stays OFF; №75 H2446 `HINDI_TG_CURATED_PRACTICE` stay OFF; №74 H2445 `HINDI_MY_SRS_DECK` stay OFF; №73 H2444 `HINDI_ATTACHMENT_DRILLS` stays OFF; H2645+H2644 клуб: `CLUB_MEMBERSHIP` к 28-08, порядок трёх флагов; №72 H2485 `CRM_SALES_FORECAST` ON; №71 H2443 `HINDI_TRANSCRIPT_DRILLS` ON; №70 H2441 `HINDI_PROGRAMME_PLAYLIST` ON; H2493 Grammar Lab G2 flags stay OFF; H2484 lifecycle flag OFF as №69; H2483 CRM 360 flag OFF as №68; H2482 VisualDCS flags stay OFF; №65 H2110 «Старт чтения» — флаг `KOSHA_READER`; H1947 «войти как» — флаг; H2085 silent-grant flags; H2017 PayPal/invoice ON; H2014 session; авто-деплой жив)_
...
| 71 | **H2443 упражнения из расшифровки хинди** | ✅ **включено 14-08-2026.** `HINDI_TRANSCRIPT_DRILLS=true` + `config:cache`. Prod HEAD `319f33e6`. Probe lesson 1863 (курс 401, Костина нач. 1): 12 cloze. Lesson 1854 (курс 402): 11 cloze + 1 vocab_pick. In-process GET `/c/hindi-1-sr800-2026/u/1863/drills` as paid user 6738 → 200; guest → 302. У user 6494 36 открытых занятий, но `transcript_file` пуст — ссылки «Упражнения» у неё не появятся, пока нет расшифровок. Откат: флаг `false` + `config:cache`. | ⚙️ флаг (без денег) |
...
| 73 | **H2444 упражнения из файлов занятия хинди** | После авто-деплоя код инертен (`HINDI_ATTACHMENT_DRILLS=false`). Включение (human): `HINDI_ATTACHMENT_DRILLS=true` + `config:cache`. На проде 14-08-2026 у хинди **0** `lessons.attachments`; 3 homework-файла (1 PDF без sidecar, 5 mp3, 1 png). Смоук: `php artisan hindi:attachments-census --json`. Откат: флаг `false` + `config:cache`. | ⚙️ флаг (без денег) |
...
| 76 | **H2731 sidecar PDF хинди (занятие 1723)** | После авто-деплоя флаг `HINDI_ATTACHMENT_DRILLS` **не трогать** (остаётся OFF). Разовый extract: `php artisan hindi:pdf-sidecar 1723 --json` (dry-run), затем `--apply`. Пишет sibling `.txt` первых 8 страниц через `gs` уже на боксе. Не Telegram, не bulk OCR. Откат sidecar: удалить `storage/app/public/homework-prompts/Практическая_грамматика_хинди_Уровень_a1.txt`. | разовый artisan (без флага) | H2444 видит текст раздатки 1723 без включения флага |
