...
| T3 | **Секрет вебхука (позже, после dev-PR).** | Когда выйдет PR с проверкой секрета для `/api/telegram/webhook`, перезапустить `setWebhook` с `secret_token` — точная команда придёт отдельным пунктом этого списка. | Защита кабинетного вебхука перед ростом трафика. |
