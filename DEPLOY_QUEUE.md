...
| 81 | **H3280 календарь выплат по неделям** | После деплоя инертен (`TEACHER_WEEKLY_PAYOUT_CALENDAR=false`). Включение: `true` + `config:cache`. Смоук: `/admin/teacher-weekly-payout-calendar` у бухгалтера; `php artisan teacher-payouts:week-calendar --json` → `money_tables_moved:false`. PayPal cover всегда «откройте PayPal». Откат: `false` + `config:cache`. | ⚙️ флаг (чтение, не проводка) |
