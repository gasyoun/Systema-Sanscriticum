...
| 13 | **Марафон — Phase 2: дрип-движок Day 1/2 через Telegram** (H464, PR TBD) | входит в общий `php artisan migrate` — миграций нет в этом слайсе; планировщик `marathon:deliver-due` (каждые 15 мин) уже прописан в `Kernel::schedule()`, сработает сам после деплоя, если серверный cron вызывает `schedule:run` (см. примечание к №4/5/8/9 выше); **дополнительно вручную**: в `MarketingSetting` (Filament) должны быть заполнены `tg_bot_username`/`tg_bot_token` для бота `@samskrte` — без них `magnet_token`-диплинк в письме/на странице ведёт на несуществующего бота | код + планировщик, миграций нет | Day 1/Day 2 контент по личному дню энрола; **Phases 3b/4/5/6 (интерактивный тап-чойс UI, платный трек ₽500, живая консультация, тёплый хвост) — отдельные будущие PR, ещё не влиты** |
