...
/**
 * ACCEPT gates for the second Arzamas longread «Россия и санскритский словарь»
 * (H1696): import is idempotent, drafts stay hidden, the published page renders
 * with >= 12 h2 chapters, and the Materials hub surfaces the card.
 *
 * Runs against the real repo pack under docs/materials/kossovich-arzamas/ — the
 * same payload production imports — so a broken build_body.py output fails CI here.
 */
class KossovichArzamasMaterialTest extends TestCase
{
    ...
    public function test_both_arzamas_longreads_coexist(): void
    {
        $this->artisan('materials:import-pwg-arzamas', ['--publish' => true])->assertSuccessful();
        $this->artisan('materials:import-kossovich-arzamas', ['--publish' => true])->assertSuccessful();

        $this->assertSame(1, Article::where('slug', 'peterburgskiy-slovar-pwg')->count());
        $this->assertSame(1, Article::where('slug', self::SLUG)->count());
    }
}
