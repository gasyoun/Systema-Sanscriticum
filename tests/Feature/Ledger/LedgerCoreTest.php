...
class LedgerCoreTest extends TestCase
{
    ...
    public function test_allocations_are_immutable_and_obligation_terms_are_frozen(): void
    {
        ...
        $this->assertLedgerRejects('write-once', fn () => $this->ledger->markDelivered($block->fresh(), now()->subDay()));
    }
    ...
    public function test_property_random_operation_sequences_never_break_invariants(): void
    {
        ...
        for ($step = 0; $step < 160; $step++) {
            ...
            try {
                if ($op === 0 || $receipts === []) {
                    ...
                    $receipts[] = $r;
                    $model[$r->id] = ['net' => $amt, 'refunded' => 0, 'allocated' => 0];
                    ...
                    // Без названных обязательств возврат берёт только свободный остаток семьи.
                    $expectOk = $ask <= $model[$r->id]['net'] - $model[$r->id]['refunded'] - $model[$r->id]['allocated'];
                    try {
                    ...
                    try {
                        ...
                        $model[$r->id]['net'] = $new;
                        // Корректировка наследует распределения в пределах нового свободного нетто.
                        $model[$r->id]['allocated'] = min($model[$r->id]['allocated'], $new - $model[$r->id]['refunded']);
                    } catch (LedgerInvariantViolation $e) {
                    ...
                    $live = MoneyMovement::query()->where(fn ($q) => $q->where('id', $r->id)->orWhere('root_movement_id', $r->id))
                        ...
                        ->whereNotExists(fn ($q) => $q->from('money_movements as x')->whereColumn('x.reverses_movement_id', 'money_movements.id'))
                        ->first();
                    $amt = mt_rand(1, 300000);
                    $this->ledger->allocate($live, $blocks[$r->id], $amt, "a{$step}");
                    $model[$r->id]['allocated'] += $amt;
                }
        ...
        foreach ($receipts as $r) {
            ...
            $this->assertSame($model[$r->id]['allocated'], $this->projection->familyAllocated($r->id), "распределено {$r->id}");
        }
        $totals = $this->projection->controlTotals();
        $this->assertSame($totals['student_money_net_kopecks'], $totals['allocated_kopecks'] + $totals['unallocated_residue_kopecks']);
