...
def find_timecodes_for_headings(
    ...
    for i, m in enumerate(matches):
        ...
        if score >= threshold:
            ...
            results[i] = (m.end(), int(sec), score_pct, algo, title)
...
def insert_timecodes(
    ...
    if still_missing:
        ...
        for i in still_missing:
            heading_start = matches[i].start()
            insert_pos    = matches[i].end()
            title         = re.sub(r'<[^>]+>', '', matches[i].group())

            # Найти ближайших левого и правого соседей по позиции заголовка
            left  = (0, 0)
            ...
            if rc == lc:
                ...
                ratio = (heading_start - lc) / (rc - lc)
                sec_interp = int(lt + ratio * (rt - lt))

            algo_tag = 'interpolated'
            insertions[i] = (insert_pos, sec_interp, 0, algo_tag, title)
