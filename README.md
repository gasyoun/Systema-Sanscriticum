...
адаптация кабинета и реферальная награда денежным кредитом — тоже закрыты.
Следующий приоритет (**P3**) — углубление геймификации праны.
...
- [~] **★ Прана — углубление геймификации** — [x] таблица лидеров по `lifetime_prana`
  (#202); [x] бейджи-достижения (#204, `config/badges.php`); [x] streak-награды за
  серию активных дней с бонусом на вехах (#206, earn-sink). [ ] осталось: новый
  **spend-sink** (трата праны на перки/контент) — требует продуктового решения
  «что покупать за прану» (скидка при оплате + P2P-подарки уже есть).
