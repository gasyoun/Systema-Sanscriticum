...
- [x] **Апгрейд тарифов** — зачёт уплаченного при докупке реализован в `Tariff::upgradeCreditForUser()` (половина блока → целый блок, блоки → полный курс).
- [ ] **Тесты** — покрытие уже не «минимальное»: ~50 файлов в `tests/`. Реальные **пробелы** в денежно-критичном ядре: вебхук Точки (`WebhookController::handleTochkaWebhook` — 0 тестов на подпись/идемпотентность), тиры лояльности (`Tariff::getDiscountPercentForUser`), сборка цены в чекауте (`PaymentController::createPayment`). См. `tests/README.md`.
- [x] **Чат поддержки** — реализован через мессенджеры: Telegram/VK-бот → лог `ChatMessage` → AI-автоответ с историей → передача куратору («позови куратора») → ответ из админ-панели Helpdesk/Dialogs. Остаётся только **веб-чат в самом кабинете** (модель и бэкенд готовы, не хватает UI на `/cabinet`).

### Среднесрочно

- [ ] **Мобильная адаптация кабинета** — текущие шаблоны верстались под десктоп.
- [ ] **Аналитика преподавателей** — есть общие маркетинговые виджеты, но нет **посуточного** дашборда «прогресс студентов конкретного преподавателя + воронка просмотров»; сигналы лежат в `LessonView`.
- [ ] **Реферальная программа** — переиспользуемая основа есть (`PromoCode`, `PranaService` для наград), но кода рефералок в `app/` пока нет; нужна модель вознаграждения (скидка/прана/выплата) — продуктовое решение.
- [ ] **Социальная авторизация** — **greenfield**: `config/social.php` хранит только ссылки соцсетей для шапки, пакета `laravel/socialite` и модели `SocialAccount` нет. Нужны OAuth client_id/secret по провайдерам.
...
- [ ] **Вебинары / live-сессии** — модель `Schedule` есть (и приём `lessons/from-zoom`), интеграция с видеоплатформой не реализована; выбор движка обсуждается в issue #78.

> **⚠️ Прана — две несовместимые концепции.** В коде **уже работает** прана как
> скидочная лояльная валюта (`config/prana.php`: 10 прана = 1 ₽, ≤ 30 % цены;
> `PranaService`, `PranaTransaction`, настройки в админке). Это **противоречит**
> отдельному документу `PRANA_ROADMAP.md` / `prana-gemini.md`, который описывает
> геймификацию (ранги Śiṣya→Paṇḍita, lifetime_prana, распад, P2P) и **явно
> запрещает** конвертацию в деньги. Тот роадмап **не реализован** и фактически
> расходится с тем, что в проде — нужно продуктовое решение (см. `.ai_state.md`).
