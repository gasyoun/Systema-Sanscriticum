...
студенту, **P1** — следующая волна, **P2** — стратегические задачи. P0 и P1
полностью закрыты; из P2 остаются редактор лекций v2, вебинары и продуктовые
развилки.
...
- [ ] **P2 · Вебинары / live-сессии (issue #78)** — есть `Schedule` с ручной
  Zoom-ссылкой, напоминания и push-импорт записей (`/api/lessons/from-zoom`).
  Дальше — автосоздание встреч через Zoom Server-to-Server OAuth и автоимпорт
  записей по вебхуку `recording.completed` (нужны доступы Zoom-приложения).
- [ ] **Follow-up · VK / Yandex socialite-драйверы** — community-пакеты
  `socialiteproviders/*` + listener (Google уже работает).

### 🤔 Продуктовые развилки (требуют решения, не автономно)

- Модель вознаграждения рефералки (сейчас фиксированные 100 праны).
- Видеоплатформа для вебинаров (Zoom выбран как движок; объём MVP — issue #78).
