...
полностью закрыты** (включая редактор лекций v2 и вебинары Zoom). Следующая
волна — **P3**, и её ведёт мобильная адаптация кабинета.
...
- [ ] **★ P3 (приоритет) · Мобильная адаптация кабинета** — адаптивная вёрстка
  существующих Blade-шаблонов `/dvaram` (решено: responsive Blade, а не PWA/нативное
  сейчас; мобильный API остаётся фундаментом на будущее).
- [ ] **Реферальная награда → денежный кредит** — заменить фиксированные 100 праны
  на реальный кредит/скидку (решено; переходит из развилки в задачу).
- [ ] **Прана — углубление геймификации** — продолжать направление рангов/lifetime:
  лидерборды, бейджи, новые сферы заработка/трат (решено: геймификацию развиваем).
...
- **Модель вознаграждения рефералки** = денежный кредит/скидка (в работе, см. «Осталось»).
- **Прана** = развивать геймификацию (скидочный кошелёк и накопительный ранг
  сосуществуют — two-counter).
